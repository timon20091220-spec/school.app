...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1120.js',{scope:'./',updateViaCache:'none'})
...
function defaultSchoolMap(floor){
  ...
  if(f===1){
    const rooms={
      'entrance-main':mapSpace('entrance-main','본관 입구','entrance',50,93,18,8,0,'1:hall-main,1:corridor-south',{background:'#dff6e8'}),
      'stairs-west':mapSpace('stairs-west','서쪽 계단','stairs',6,62,9,13,0,'1:room-1-1,1:office,2:stairs-west',{background:'#dbeafe'}),
      'room-1-1':mapSpace('room-1-1','1-1','room',8,15,13,13,0,'1:stairs-west,1:room-1-2'),
      'room-1-2':mapSpace('room-1-2','1-2','room',8,30,13,13,0,'1:room-1-1,1:office'),
      'office':mapSpace('office','교무실','room',8,46,13,14,0,'1:room-1-2,1:stairs-west,1:hall-main',{background:'#fff2cc'}),
      'restroom-west':mapSpace('restroom-west','화장실','restroom',20,12,9,10,0,'1:hall-main',{background:'#fff1d6'}),
      'hall-main':mapSpace('hall-main','중앙 행사장','hall',48,35,43,34,0,'1:entrance-main,1:restroom-west,1:student-council,1:broadcast,1:health,1:science,1:gym',{background:'#f7f3e8'}),
      'student-council':mapSpace('student-council','학생회실','room',25,69,16,11,-18,'1:hall-main,1:broadcast'),
      'broadcast':mapSpace('broadcast','방송실','room',42,74,15,11,-10,'1:student-council,1:hall-main,1:health'),
      'health':mapSpace('health','보건실','room',59,72,15,11,10,'1:broadcast,1:hall-main,1:science'),
      'science':mapSpace('science','과학실','room',74,66,15,12,18,'1:health,1:hall-main,1:stairs-east'),
      'gym':mapSpace('gym','체육관','hall',89,36,14,48,0,'1:hall-main,1:stairs-east',{background:'#f0e8ff'}),
      'stairs-east':mapSpace('stairs-east','동쪽 계단','stairs',91,74,9,13,0,'1:gym,1:science,2:stairs-east',{background:'#dbeafe'}),
      'corridor-south':mapSpace('corridor-south','남쪽 복도','corridor',50,84,58,6,0,'1:entrance-main,1:student-council,1:broadcast,1:health,1:science',{background:'#e8edf3'})
    };
    return {
      id:'floor-1',floor:1,title:'1층 안내도',background:'#f5f2ea',
      backgroundImage:'',layoutVersion:2,points:rooms
    };
  }
  ...
  const roomCount=7;
  const rooms={};
  rooms['stairs-west']=mapSpace('stairs-west','서쪽 계단','stairs',5,50,8,24,0,`${f-1}:stairs-west,${f+1}:stairs-west`,{background:'#dbeafe'});
  rooms['corridor']=mapSpace('corridor','중앙 복도','corridor',50,62,82,10,0,`${f}:stairs-west,${f}:stairs-east`,{background:'#e8edf3'});
  for(let index=0;index<roomCount;index++){
    const roomNo=index+1;
    const id=`room-${grade}-${roomNo}`;
    const x=15+index*11.3;
    const previous=index===0?`${f}:stairs-west`:`${f}:room-${grade}-${roomNo-1}`;
    const next=index===roomCount-1?`${f}:stairs-east`:`${f}:room-${grade}-${roomNo+1}`;
    rooms[id]=mapSpace(id,`${grade}-${roomNo}`,'room',x,32,10,24,0,`${previous},${next},${f}:corridor`);
  }
  rooms['restroom']=mapSpace('restroom','화장실','restroom',83,32,8,24,0,`${f}:corridor,${f}:stairs-east`,{background:'#fff1d6'});
  rooms['stairs-east']=mapSpace('stairs-east','동쪽 계단','stairs',94,50,8,24,0,`${f-1}:stairs-east,${f+1}:stairs-east`,{background:'#dbeafe'});
  rooms['special-room']=mapSpace(
    'special-room',
    f===2?'시청각실':'진로활동실',
    'room',50,82,24,15,0,`${f}:corridor`,
    {background:f===2?'#f4e8ff':'#e7f5ff'}
  );
  return {
    id:`floor-${f}`,floor:f,title:`${f}층 안내도`,
    background:'#f7f5ef',backgroundImage:'',layoutVersion:2,points:rooms
  };
}
...
/* v11.20 단일 저장 이벤트 보장 */
function installSingleTimetableSaveHandler(){
  const oldButton=$('#saveAdminTimetable');
  if(!oldButton)return;
  ...
  button.addEventListener('click',async event=>{
    ...
    try{
      ...
      button.disabled=false;
      button.textContent='시간표 저장';
    }
  });
}
installSingleTimetableSaveHandler();
