...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1113.js',{scope:'./',updateViaCache:'none'})
...
function waitForInstallPrompt(timeout=12000){
...
$('#installButton').addEventListener('click',async()=>{
  ...
  button.title='설치 준비 중';

  try{
    await ensureServiceWorker();
    const promptEvent=await waitForInstallPrompt(12000);
    ...
    if(isIOS){
      toast('iPhone·iPad는 Safari 공유 버튼의 홈 화면에 추가를 사용해주세요.');
    }else{
      toast('설치 기능이 아직 준비되지 않았습니다. 잠시 후 설치 버튼을 다시 눌러주세요.');
    }
  }finally{
    button.disabled=false;
    button.classList.remove('install-waiting');
    button.title=oldTitle||'앱 설치';
  }
});
...
$('#confirmGuestEntry').addEventListener('click',async()=>{
  ...
  try{
    ...
    toast(firebaseLoadMessage(error));
  }finally{
    button.disabled=false;
...
function notificationMatchesAudience(item){
  ...
  if(audience==='전체'||audience==='개인')return true;
