/* 배문고 학교 앱 Service Worker v13.01 · v11.44 notification flow */
const CACHE_NAME='baemoon-school-app-v13.01';
const CORE_ASSETS=[
  './','./index.html','./styles-v1301.css','./app-v1301.js','./firebase-runtime-v1301.js',
  './manifest.webmanifest?v=1301','./privacy.html','./icons/icon-192.png','./icons/icon-512.png'
];
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil((async()=>{
    ...
    for(const client of windows){
      if(client.url.startsWith(self.registration.scope)){if('navigate'in client&&client.url!==target)await client.navigate(target).catch(()=>{});await client.focus();client.postMessage({type:'QUEUE_NOTIFICATION_OPENED',queueId:event.notification.data?.queueId||''});return;}
    }
