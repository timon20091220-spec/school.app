...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],notifications:[],
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.24 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.24 Firestore 규칙을 게시해주세요.';
...
onAuthStateChanged(auth,async user=>{
  ...
  try{
    ...
    if(user.isAnonymous){
      ...
      await updateDoc(doc(db,'guestProfiles',user.uid),{lastLoginAt:now,deviceId});
      ui().setSession({...guestProfile,deviceId,lastLoginAt:now});
