...
const state={
  ...
  suggestionMode:'public',currentSuggestionId:null,
...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1119.js',{scope:'./',updateViaCache:'none'})
...
$('#saveAdminTimetable').addEventListener('click',async()=>{
  const grade=Number($('#adminTimetableGrade').value);
  const classNo=Number($('#adminTimetableClass').value);
  const days=Array.from({length:5},()=>Array(7).fill(''));
  $$('[data-timetable-subject]').forEach(input=>{
    const [day,period]=input.dataset.timetableSubject.split(':').map(Number);
    days[day][period]=input.value.trim();
  });
  try{
    await window.baemoonFirebase.saveTimetable({id:timetableDocumentId(grade,classNo),grade,classNo,days});
...
$('#classChatForm').addEventListener('submit',async event=>{
  ...
  button.disabled=true;
  try{
    await window.baemoonFirebase.sendClassChatMessage({classKey:classChatKey(),body});
    input.value='';
  }catch(error){toast(firebaseFriendlyMessage(error))}
...
$('#saveAdminTimetable').addEventListener('click',async event=>{
  event.preventDefault();
  event.stopImmediatePropagation();
  const grade=Number($('#adminTimetableGrade').value);
  const classNo=Number($('#adminTimetableClass').value);
  ...
  $$('[data-timetable-subject]').forEach(input=>{
    ...
    days[dayKey][Number(periodText)]=input.value.trim();
  });
  const button=$('#saveAdminTimetable');
  button.disabled=true;
  button.textContent='저장 중…';
  try{
    await window.baemoonFirebase.saveTimetable({
      id:timetableDocumentId(grade,classNo),grade,classNo,days
    });
    toast(`${grade}학년 ${classNo}반 시간표를 저장했습니다.`);
  }catch(error){toast(firebaseFriendlyMessage(error))}
  finally{button.disabled=false;button.textContent='시간표 저장'}
},true);
...
function floorMap(floor){
  const existing=schoolMaps().find(item=>Number(item.floor)===Number(floor));
  return existing||{id:`floor-${floor}`,floor:Number(floor),title:`${floor}층 안내도`,image:'',points:{}};
}
...
function allMapPoints(){
  return [1,2,3].flatMap(floor=>mapPoints(floorMap(floor)));
}
function mapPointByGlobalId(globalId){
  return allMapPoints().find(point=>point.globalId===globalId);
}
function renderRouteSelectors(){
  const points=allMapPoints().sort((a,b)=>a.floor-b.floor||String(a.name).localeCompare(String(b.name),'ko'));
  ...
  if(points.some(p=>p.globalId===startValue))$('#routeStart').value=startValue;
  if(points.some(p=>p.globalId===endValue))$('#routeEnd').value=endValue;
}
...
function renderGuide(){
  ...
  $('#mapPointCount').textContent=`${points.length}개 위치`;
  const image=imageSrc(map.image);
  $('#floorMapImage').src=image||'';
  $('#floorMapImage').hidden=!image;
  $('#mapEmptyState').hidden=!!image;
  $('#mapPointLayer').innerHTML=points.map(point=>`
    <button class="map-location-pin map-type-${esc(point.type||'room')}"
      style="left:${Number(point.x||50)}%;top:${Number(point.y||50)}%"
      data-map-point="${esc(point.globalId)}" title="${esc(point.name)}">
      <i></i><span>${esc(point.name)}</span>
    </button>`).join('');
  ...
  if(segment.length>=2){
    ...
    overlay.innerHTML=`<polyline points="${path}" class="map-route-line"></polyline>`+
      segment.map((point,index)=>`<circle cx="${Number(point.x||50)*10}" cy="${Number(point.y||50)*7}" r="${index===0||index===segment.length-1?13:8}" class="map-route-node"></circle>`).join('');
...
function openMapPointDetail(globalId){
  ...
  $('#mapPointDetailFloor').textContent=`${point.floor}F · ${point.type==='stairs'?'계단':point.type==='elevator'?'엘리베이터':'시설'}`;
  $('#mapPointDetailName').textContent=point.name;
  $('#mapPointDetailDescription').textContent=point.description||'등록된 상세 설명이 없습니다.';
  const src=imageSrc(point.photo);
...
$('#mapPointSetDestination').addEventListener('click',()=>{
  const id=state.mapPointDetailId;
  if(!id)return;
  $('#routeEnd').value=id;
...
function buildMapGraph(){
  ...
  points.forEach(point=>{
    String(point.links||'').split(',').map(value=>value.trim()).filter(Boolean).forEach(target=>link(point.globalId,target));
  });
  [1,2,3].forEach(floor=>{
    const floorPoints=points.filter(point=>point.floor===floor);
    floorPoints.forEach(point=>{
      floorPoints.filter(other=>other.globalId!==point.globalId)
        .sort((a,b)=>edgeDistance(point,a)-edgeDistance(point,b))
        .slice(0,3).forEach(other=>link(point.globalId,other.globalId));
    });
  });
  const connectors=points.filter(point=>['stairs','elevator'].includes(point.type));
  connectors.forEach(point=>{
    connectors.filter(other=>
      other.globalId!==point.globalId&&
      other.type===point.type&&
      (String(other.name).replace(/\d/g,'')===String(point.name).replace(/\d/g,'')||Math.abs(other.floor-point.floor)===1)
    ).forEach(other=>link(point.globalId,other.globalId));
  });
...
function routeStepText(path){
  ...
  for(let index=1;index<path.length;index++){
    ...
    if(before.floor!==current.floor){
      const method=['elevator'].includes(before.type)||['elevator'].includes(current.type)?'엘리베이터':'계단';
...
function runRouteSearch(){
  ...
  if(!path.length)return toast('연결된 경로를 찾지 못했습니다. 관리자 지도에서 위치 연결을 확인해주세요.');
  ...
  $('#routeResult').innerHTML=`<span class="route-number">${path.length}</span><div>
    ...
    <small>${path.length}개 위치를 연결한 추천 경로입니다.</small></div>`;
  $('#routeStepList').innerHTML=steps.map((step,index)=>`<article><span>${index+1}</span><p>${esc(step)}</p></article>`).join('');
  renderGuide();
  toast('추천 경로를 표시했습니다.');
}
...
function ensureAdminMapDraft(floor=state.adminMapFloor){
  ...
  if(!state.adminMapDrafts[key]){
    const source=floorMap(floor);
    state.adminMapDrafts[key]=JSON.parse(JSON.stringify({
      id:`floor-${floor}`,floor:Number(floor),title:source.title||`${floor}층 안내도`,
      image:source.image||'',points:source.points||{}
    }));
  }
...
function renderAdminMap(){
  ...
  $('#adminMapPreviewImage').src=src||'';
  $('#adminMapPreviewImage').hidden=!src;
  $('#adminMapPreviewEmpty').hidden=!!src;
  const points=mapPoints(draft);
  $('#adminMapPreviewPoints').innerHTML=points.map(point=>`
    <button style="left:${Number(point.x||50)}%;top:${Number(point.y||50)}%"
      data-edit-map-point="${esc(point.id)}" title="${esc(point.name)}">
      <i></i><span>${esc(point.name)}</span>
    </button>`).join('');
  $('#adminMapPointList').innerHTML=points.length?points.map(point=>`
    <article>
      <div><b>${esc(point.name)}</b><small>${esc(point.type||'room')} · ${Number(point.x).toFixed(1)}%, ${Number(point.y).toFixed(1)}%</small></div>
      <div><button data-edit-map-point="${esc(point.id)}">수정</button><button class="danger" data-delete-map-point="${esc(point.id)}">삭제</button></div>
    </article>`).join(''):'<div class="admin-empty">등록된 위치가 없습니다.</div>';
  $$('[data-edit-map-point]').forEach(button=>button.addEventListener('click',event=>{
    event.stopPropagation();openMapPointEditor(button.dataset.editMapPoint);
  }));
...
function renderMapPointLinkOptions(currentId=''){
  ...
  $('#mapPointLinkOptions').innerHTML=allDraftMapPoints()
    ...
    .map(point=>`<label><input type="checkbox" value="${esc(point.globalId)}" ${selected.has(point.globalId)?'checked':''}>
      <span>${point.floor}F · ${esc(point.name)}</span></label>`).join('')||'<small>먼저 다른 위치를 등록해주세요.</small>';
}
function openMapPointEditor(pointId=null,position=null){
  ...
  $('#mapPointEditorTitle').textContent=point?'위치 수정':'위치 추가';
  ...
  state.pendingImage.mapPoint=point?.photo||'';
...
$('#adminMapImageInput').addEventListener('change',async event=>{
  try{
    const draft=ensureAdminMapDraft();
    draft.image=await compressImage(event.target.files?.[0]);
    renderAdminMap();
    toast(`${state.adminMapFloor}층 지도 이미지를 준비했습니다.`);
  }catch(error){toast(error.message)}
});
$('#adminMapPreview').addEventListener('click',event=>{
  if(event.target.closest('[data-edit-map-point]'))return;
  const image=$('#adminMapPreviewImage');
  if(image.hidden)return toast('먼저 지도 이미지를 등록해주세요.');
...
$('#saveMapPointButton').addEventListener('click',()=>{
  ...
  if(!name)return toast('위치 이름을 입력해주세요.');
...
$('#saveAdminMapFloor').addEventListener('click',async()=>{
  ...
  try{
    ...
    state.adminMapDrafts[String(state.adminMapFloor)]=JSON.parse(JSON.stringify(saved));
    toast(`${state.adminMapFloor}층 지도를 저장했습니다.`);
  }catch(error){toast(firebaseFriendlyMessage(error))}
