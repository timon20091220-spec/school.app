...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1126.js',{scope:'./',updateViaCache:'none'})
...
$('#confirmReservation').addEventListener('click',async()=>{
  ...
  try{
    ...
    if(result?.outcome==='waitlisted'){
      toast(`${booth.name} 예약이 마감되어 대기번호 ${Number(result.queue?.queueNumber||0)||'-'}번으로 접수됐습니다.`);
    }else{
      toast(`${booth.name} ${groupSize}명 예약이 확정되었습니다.`);
    }
...
function updateReservationPartyAvailability(festival,booth,time){
  ...
  $('#reservationPartySizeHint').textContent=fullForNew
    ?canQueue
      ?`현재 ${availability.reserved}/${availability.capacity}명 · 예약 시 대기번호로 자동 전환됩니다.`
...
/* ===== v11.26.2 launch-ready overrides ===== */
...
/* v11.26 시간표 저장: 문서 캡처 단계에서 단 한 번만 처리 */
...
/* v11.26 부스 저장: 즉시 예약 허용 + 단일 저장 이벤트 */
document.addEventListener('click',async event=>{
...
route=function(screen,opts={}){
  ...
  if(screen==='admin'&&isAdmin())window.baemoonFirebase?.getAdminStats?.().catch(()=>{});
  window.scrollTo({top:0,behavior:'smooth'});
};
...
function renderAdminStats(){
  ...
  $('#adminBoothRanking').innerHTML=ranking.length?ranking.map((item,index)=>`
    ...
    <small>${esc(item.festival_name||item.festivalName||'행사')} · ${esc(formatReservationSlot(item.time||'즉시 예약'))} · 예약 ${Number(item.reserved_people||item.reservedPeople||0)}명 · 체크인 ${Number(item.checked_in_people||item.checkedInPeople||0)}명</small></div>
    <i>서버 집계</i></article>`).join(''):'<div class="admin-empty">통계에 표시할 서버 예약이 없습니다.</div>';
...
$('#refreshAdminStats').addEventListener('click',async()=>{
  ...
  try{await window.baemoonFirebase.getAdminStats();toast('Cloudflare 서버 통계를 갱신했습니다.');}
...
$('#syncReservationServerButton').addEventListener('click',async()=>{
  ...
  try{await window.baemoonFirebase.syncReservationSlots(festivals());await window.baemoonFirebase.getAdminStats();}
...
openAdminBoothReservations=async function(key){
  ...
  try{
    const items=await window.baemoonFirebase.getAdminReservations({festivalId:entry.festival.id,boothId:entry.booth.id});
    const others=reservations().filter(item=>!(item.festivalId===entry.festival.id&&item.boothId===entry.booth.id));
    window.firebaseCache.reservations=[...others,...items];
    openAdminBoothReservationsV1124(key);
    const time=normalizedBoothTimes(entry.booth,entry.festival)[0]||'즉시 예약';
    const queue=await window.baemoonFirebase.getAdminSlotQueue({festivalId:entry.festival.id,boothId:entry.booth.id,time});
