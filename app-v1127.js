...
(()=>{
  ...
  function patchVersionLabels(){
    ...
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.35')}
...
(()=>{
  ...
  (async()=>{if(!await waitReady())return;installDetailsToggles();manualQrOnly();await syncMine({notify:false});scheduleSync(4000);window.addEventListener('focus',()=>syncMine());window.addEventListener('online',()=>syncMine());console.info('Baemoon v11.35 ready')})();
  document.documentElement.dataset.baemoonVersion=VERSION;
})();
