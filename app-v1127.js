...
if('serviceWorker' in navigator){
  navigator.serviceWorker.addEventListener('controllerchange',()=>{
    // v11.30: 새 서비스 워커가 설치되어도 사용 중인 화면을 강제로 새로고침하지 않습니다.
...
async function processQrCheckIn(code){
  ...
  try{
    ...
    renderAdminStats();renderAdminReservations();
    return true;
...
(()=>{
  ...
  function patchVersionLabels(){
    ...
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.30')}
  ...
  (async()=>{
    ...
    }catch(error){console.error('v11.30 initialization failed:',error)}
  })();
})();
