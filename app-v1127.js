...
(()=>{
  ...
  function patchVersionLabels(){
    ...
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.34')}
...
(()=>{
  ...
  function scheduleSync(delay){clearTimeout(syncTimer);syncTimer=setTimeout(async()=>{if(!document.hidden)await syncMine();const fast=document.querySelector('#reservationDetailModal.open,#reservationQrModal.open')||window.baemoonApp?.state?.screen==='my';scheduleSync(fast?3000:12000)},delay)}
  ...
  (async()=>{if(!await waitReady())return;installDetailsToggles();manualQrOnly();await syncMine({notify:false});scheduleSync(4000);window.addEventListener('focus',()=>syncMine());window.addEventListener('online',()=>syncMine());console.info('Baemoon v11.34 ready')})();
  document.documentElement.dataset.baemoonVersion=VERSION;
})();
