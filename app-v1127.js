...
(()=>{
  ...
  function setVersionV1201(){
    ...
    document.querySelectorAll('[data-app-version],.app-version,.firebase-beta-status+small,.auth-welcome-card>small').forEach(element=>{
      ...
      if(/v11\.(?:27|33|34|35|36|42|43|44)/.test(text))element.textContent=text.replace(/v11\.(?:27|33|34|35|36|42|43|44)/g,'v12.04');
    });
  }
  ...
  (async()=>{
    ...
    console.info('Baemoon v12.04 features ready');
  })();
})();
...
(()=>{
  ...
  window.addEventListener('change',event=>{if(event.target?.matches?.('#notificationToggle')){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();(window.__toggleNotificationsV1204?window.__toggleNotificationsV1204(event.target.checked):toggleNotifications1202(event.target.checked))}},true);
  window.addEventListener('click',event=>{
  ...
  function ui1202(){
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.01)/g,'v12.04'));
    const logoutText=document.querySelector('#logoutButton b');if(logoutText)logoutText.textContent='로그아웃';
  ...
  (async()=>{
    ...
    console.info('Baemoon v12.04 features ready');
  })().catch(error=>console.error('v12.04 initialization failed',error));
})();


/* ===== v12.04 admin rosters · check-in refresh · guest resume · layout fixes ===== */
(()=>{
  ...
  async function refreshMyExperience1203({force=false,render=true}={}){
    ...
    try{
      ...
      return payload;
    }catch(error){console.warn('v12.04 my overview refresh failed',error);return null}finally{myRefreshBusy1203=false}
  }
  ...
  function ui1203(){
    document.documentElement.dataset.baemoonVersion='12.04';
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.0[12])/g,'v12.04'));
  }
...
/* ===== v12.04 guest account security · queue/push reliability · admin interaction fixes ===== */
(()=>{
  'use strict';
  ...
  function ensureGuestPasswordUi(){
    ...
    let wrap=card.querySelector('#guestPasswordAreaV1204');
    if(!wrap){
      wrap=document.createElement('div');wrap.id='guestPasswordAreaV1204';wrap.className='guest-password-area-v1204';
      wrap.innerHTML='<label class="guest-password-field-v1204"><span>게스트 비밀번호</span><input id="guestPasswordV1204" type="password" autocomplete="current-password" placeholder="비밀번호 입력"></label><div id="guestFirstPasswordV1204"><p><b>최초 로그인</b> 최초 비밀번호 <code>baemoon2026</code>을 입력한 뒤 개인 비밀번호를 설정합니다.</p><label class="guest-password-field-v1204"><span>개인 비밀번호</span><input id="guestNewPasswordV1204" type="password" minlength="8" autocomplete="new-password" placeholder="8자 이상"></label><label class="guest-password-field-v1204"><span>개인 비밀번호 확인</span><input id="guestNewPasswordConfirmV1204" type="password" minlength="8" autocomplete="new-password" placeholder="한 번 더 입력"></label></div><small id="guestAccountRuleV1204">한 브라우저에서는 하나의 게스트 계정만 사용할 수 있습니다.</small>';
    ...
    const lock=guestLock(),school=card.querySelector('#guestSchool'),name=card.querySelector('#guestName'),first=card.querySelector('#guestFirstPasswordV1204'),notice=card.querySelector('#guestAccountRuleV1204'),button=card.querySelector('#confirmGuestEntry'),keep=card.querySelector('.keep-login-v1202');
    if(keep)keep.hidden=true;
    if(lock){if(school){school.value=lock.school||'';school.readOnly=true}if(name){name.value=lock.name||'';name.readOnly=true}if(first)first.hidden=true;if(notice)notice.textContent=`이 브라우저의 기존 게스트 계정(${lock.school||''} ${lock.name||''})으로만 로그인할 수 있습니다.`;if(button)button.textContent='게스트 로그인'}
    else{if(school)school.readOnly=false;if(name)name.readOnly=false;if(first)first.hidden=false;if(notice)notice.textContent='최초 등록 후 이 브라우저에서는 다른 게스트 계정을 만들 수 없습니다.'}
  }
  ...
  function cleanupAdminToggle(){const button=document.querySelector('#festivalAdminQuickToggleV1203');if(button){if(!button.hidden)button.hidden=true;if(button.style.display!=='none')button.style.display='none';if(button.getAttribute('aria-hidden')!=='true')button.setAttribute('aria-hidden','true')}}
  ...
  async function repairPush(){if(localStorage.getItem(NOTIFY_KEY)==='0'||!window.baemoonApp?.session?.()||!('Notification'in window)||Notification.permission!=='granted')return;try{await enablePush({silent:true})}catch{}}
