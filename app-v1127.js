...
let activeSession=null;try{localStorage.removeItem(KEYS.session)}catch{}
const state={
...
(()=>{
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      toast(number?`${booth.name} 예약 완료 · 번호 ${number}번`:`${booth.name} 예약이 완료되었습니다.`);
...
(()=>{
  ...
  renderFestival=function(){baseRenderFestival();prepareContinuousFestival();openSelectedFestivalAdmin();setTimeout(()=>{ensureParticipationDeleteButton();renderAdminReviews(false)},0)};
  ...
  renderMy=function(){baseRenderMy();renderMyParticipations()};
...
(()=>{
  ...
  function installApi(){
    ...
    api.callQueueEntry=async queueId=>{
      ...
      if(result?.queue){const queue=updateQueueCache(result.queue);if(!result.limitReached)await writePersonalNotification(queue,{title:'대기번호가 호출되었습니다',body:`${queue.boothName} · 대기번호 ${queue.queueNumber}번 · ${Number(result.callCount||queue.callCount||1)}/3 호출입니다. QR을 준비하고 부스로 이동해주세요.`,type:'queue'});}
      return result;
    };
    api.cancelQueueCall=async queueId=>{
      ...
      if(result?.queue){const queue=updateQueueCache(result.queue);if(!result.alreadyCancelled)await writePersonalNotification(queue,{title:'호출 취소 안내',body:CANCEL_CALL_MESSAGE,type:'queue-cancel'});}
      return result;
    };
  ...
  renderMy=function(){baseRenderMyV1143?.();hideReservationCardsInMy();renderCompactMyQueue();renderMyParticipationsCompact();enhanceQrUi()};
  ...
  renderFestival=function(){baseRenderFestivalV1143?.();keepFestivalEditorOutOfDetail();ensureFestivalAdminToggle();setTimeout(()=>renderAdminReviewsCompact(false),0)};
  ...
  function runUi(){
    ...
    if(window.baemoonApp?.state?.screen==='my'){hideReservationCardsInMy();renderCompactMyQueue();renderMyParticipationsCompact()}
    if(window.baemoonApp?.state?.screen==='festivals')ensureFestivalHubAdmin();
    if(window.baemoonApp?.state?.screen==='festival'){ensureFestivalAdminToggle();renderAdminReviewsCompact(false)}
    document.documentElement.dataset.baemoonVersion=VERSION;
...
(()=>{
  ...
  function setReviewRating(value){
    const overlay=ensureReviewDialogV1144(),rating=Math.min(5,Math.max(1,Number(value||5)));overlay.dataset.rating=String(rating);
    ...
    const text=overlay.querySelector('#reviewRatingTextV1144');if(text)text.textContent=`${rating}점`;
  }
  ...
  function openReviewV1144(participationId){
    ...
    overlay.querySelector('#reviewTextV1136').value=existing?.review_text||existing?.review||'';setReviewRating(Number(existing?.rating||5));overlay.hidden=false;
    setTimeout(()=>overlay.querySelector('#reviewTextV1136')?.focus(),30);
  }
  async function saveReviewV1144(){
    const overlay=ensureReviewDialogV1144(),participationId=overlay.dataset.participationId,text=overlay.querySelector('#reviewTextV1136').value.trim(),rating=Number(overlay.dataset.rating||5),button=overlay.querySelector('#saveReviewV1136');
  ...
  async function renderAdminReviewsV1144(force=false){
    ...
    try{
      ...
      list.innerHTML=items.length?items.map(item=>{
        ...
        return `<article class="admin-review-card-v1143 admin-review-card-v1144"><button type="button" class="admin-review-summary-v1144" data-toggle-review-v1144="${escV(item.id)}"><span><small>${item.participant_role==='guest'?'게스트':'학생'}</small><b>${escV(person.id)}</b></span><span><small>이름</small><b>${escV(person.name)}</b></span><i>후기 보기</i></button><div class="admin-review-detail-v1144" data-review-detail-v1144="${escV(item.id)}" hidden>${stars(item.rating)}<span>${escV(item.festival_name||'행사')} · ${escV(item.booth_name||'부스')}</span><p>${escV(item.review_text||'')}</p><button type="button" data-delete-review-v1143="${escV(item.id)}">후기 삭제</button></div></article>`;
  ...
  function runUiV1144(){
    renderMyParticipationsV1144();decorateParticipationSquares();enhanceAdminQrV1144();enhancePushDiagnosticsV1144();ensureReviewDialogV1144();
    if(window.baemoonApp?.state?.screen==='festival')renderAdminReviewsV1144(false);
    document.documentElement.dataset.baemoonVersion=VERSION;
  ...
  window.addEventListener('click',event=>{
    ...
    if(review){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();openReviewV1144(review.dataset.writeReviewV1136);return}
    const save=event.target.closest?.('#saveReviewV1136');
    if(save){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();saveReviewV1144();return}
    ...
    if(toggle){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();const detail=document.querySelector(`[data-review-detail-v1144="${CSS.escape(toggle.dataset.toggleReviewV1144)}"]`);if(detail){detail.hidden=!detail.hidden;toggle.querySelector('i').textContent=detail.hidden?'후기 보기':'후기 닫기'}return}
    ...
    if(diagnose){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();runPushDiagnosticsV1144(diagnose);return}
  ...
  if(typeof renderMy==='function'){
    ...
    renderMy=function(){baseRenderMyV1144();renderMyParticipationsV1144()};
  ...
  if(typeof renderFestival==='function'){
    ...
    renderFestival=function(){baseRenderFestivalV1144();setTimeout(()=>{decorateParticipationSquares();renderAdminReviewsV1144(false)},0)};
