...
function openReservation(boothId){
  ...
  if(existing){
    toast('이미 이 부스를 예약했습니다. 한 부스는 한 번만 예약할 수 있습니다.');
...
function renderHomeBanners(){
  ...
  action.hidden=!(item.link&&item.buttonLabel);
  action.textContent=item.buttonLabel||'자세히 보기';
...
(()=>{
  ...
  function ensureAdminUtilityButtons(){
    ...
    if(!tools){
      ...
      tools.innerHTML='<summary>데이터 관리</summary><div><button type="button" id="cleanupOrphansV1130">남은 예약 정리</button><button type="button" id="resetTestReservationsV1130">테스트 예약 초기화</button></div>';
      ...
      tools.querySelector('#resetTestReservationsV1130').addEventListener('click',async()=>{
        const phrase=await appDialogV1130({title:'테스트 예약 초기화',message:'예약·번호·참여 기록을 삭제합니다.',confirmText:'초기화',input:true,placeholder:'초기화 입력'});
  ...
  function installBannerBehavior(){
    document.addEventListener('click',event=>{const action=event.target.closest('#homeHeroAction');if(!action)return;const href=String(action.getAttribute('href')||'');if(!href||href==='#')return;event.preventDefault();event.stopPropagation();window.open(href,'_blank','noopener,noreferrer')},true);
  }
  function patchVersionLabels(){
    if(versionPatched)return;
    const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.33')}
  ...
  (async()=>{
    try{
      await waitForRuntime();installApiExtensions();installBannerBehavior();observeUi();startCompletionPollingV1130();
...
/* ===== v11.33 active reservation cleanup ===== */
(()=>{
  const VERSION='11.33';
  let syncing=false,lastSignature='';
  const ids=list=>(list||[]).map(item=>String(item.id||'')).sort().join('|');
  function applyEditorCleanup(){
    const booking=document.querySelector('#boothBookingStatus');
    if(booking){booking.value='open';const label=booking.closest('label');if(label)label.hidden=true;}
    const congestion=document.querySelector('#boothCongestion');
    if(congestion){congestion.value='auto';const label=congestion.closest('label');if(label)label.hidden=true;}
  }
  function explainManualQr(){
    const input=document.querySelector('#manualQrCodeInput');
    if(!input||input.closest('details'))return;
    const label=input.closest('label'),button=document.querySelector('#confirmQrCheckInButton');
    if(!label||!button)return;
    const details=document.createElement('details');details.className='manual-qr-fallback-v1132';
    const summary=document.createElement('summary');summary.textContent='카메라가 안 될 때 코드 직접 입력';
    label.parentElement.insertBefore(details,label);details.append(summary,label,button);
    const subtitle=document.querySelector('#qrScannerModal .modal-subtitle');
    if(subtitle)subtitle.textContent='예약자의 QR을 카메라로 스캔하세요.';
  }
  function pruneClientCache(){
    const reservations=window.firebaseCache?.reservations||[];
    const queue=window.firebaseCache?.queueEntries||[];
    window.firebaseCache.reservations=reservations.filter(item=>reservationScheduleActive(item));
    window.firebaseCache.queueEntries=queue.filter(item=>['waiting','called'].includes(item.status)&&reservationScheduleActive(item));
  }
  async function reconcileMine(force=false){
    const current=window.baemoonApp?.session?.();
    if(!current?.uid||!['student','guest'].includes(current.role)||syncing)return;
    syncing=true;
    try{
      const result=await window.baemoonFirebase?.getMyServerOverview?.();
      if(!result)return;
      const normalizeReservation=row=>({
        id:String(row.id||''),slotId:String(row.slot_key||''),festivalId:String(row.festival_id||''),festivalName:String(row.festival_name||''),boothId:String(row.booth_id||''),boothName:String(row.booth_name||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||''),participantRole:String(row.participant_role||''),user:String(row.display_name||'예약자'),name:String(row.display_name||'예약자'),guestCode:String(row.guest_code||''),groupSize:Number(row.group_size||1),status:'confirmed',checkInCode:String(row.check_in_code||''),checkedIn:false,checkedInAt:null,createdAt:Number(row.created_at||Date.now()),updatedAt:Number(row.updated_at||Date.now())
      });
      const normalizeQueue=row=>({id:String(row.id||''),slotId:String(row.slot_key||''),festivalId:String(row.festival_id||''),festivalName:String(row.festival_name||''),boothId:String(row.booth_id||''),boothName:String(row.booth_name||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||''),participantRole:String(row.participant_role||''),displayName:String(row.display_name||'예약자'),guestCode:String(row.guest_code||''),queueNumber:Number(row.queue_number||0),status:String(row.status||'waiting')==='called'?'called':'waiting',createdAt:Number(row.created_at||Date.now()),calledAt:Number(row.called_at||0)||null,aheadCount:Number(row.aheadCount||0),callCount:Number(row.call_count||0)});
      const nextReservations=(result.reservations||[]).map(normalizeReservation).filter(item=>reservationScheduleActive(item));
      const nextQueue=(result.queue||[]).map(normalizeQueue).filter(item=>reservationScheduleActive(item));
      const signature=`${ids(nextReservations)}::${ids(nextQueue)}::${nextQueue.map(item=>item.status+item.queueNumber).join('|')}`;
      if(force||signature!==lastSignature){
        lastSignature=signature;
        window.firebaseCache.reservations=nextReservations;
        window.firebaseCache.queueEntries=nextQueue;
        window.baemoonApp?.renderHomeReservations?.();
        window.baemoonApp?.renderMy?.();
      }
    }catch{}finally{syncing=false}
  }
  function refreshVisibleUi(){
    pruneClientCache();applyEditorCleanup();explainManualQr();
    if(window.baemoonApp?.state?.screen==='my')reconcileMine();
  }
  document.addEventListener('click',event=>{
    if(event.target.closest('#openQrScannerButton,[data-queue-checkin]'))setTimeout(explainManualQr,0);
  },true);
  const observer=new MutationObserver(()=>requestAnimationFrame(refreshVisibleUi));
  observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','hidden']});
  setInterval(()=>{refreshVisibleUi();reconcileMine();},6000);
  window.addEventListener('focus',()=>reconcileMine(true));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)reconcileMine(true)});
  window.addEventListener('baemoon:app-ready',refreshVisibleUi,{once:true});
  setTimeout(refreshVisibleUi,0);
  document.documentElement.dataset.baemoonVersion=VERSION;
})();


/* ===== v11.33 KakaoTalk WebView stability · idempotent check-in UI ===== */
(()=>{
  const VERSION='11.33';
  ...
  function stopCameraTracksV1133(){
    document.querySelectorAll('video').forEach(video=>{
      try{video.srcObject?.getTracks?.().forEach(track=>track.stop())}catch{}
      try{video.srcObject=null}catch{}
    });
    try{window.baemoonApp?.state&&(window.baemoonApp.state.qrScanActive=false)}catch{}
  }
  function externalBrowserIntentV1133(){
    const target=new URL(location.href);target.searchParams.set('external','1');
    const clean=target.href.replace(/^https?:\/\//,'');
    const fallback=encodeURIComponent(target.href);
    return `intent://${clean}#Intent;scheme=https;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;S.browser_fallback_url=${fallback};end`;
  }
  async function openExternalV1133(){
    const target=new URL(location.href);target.searchParams.set('external','1');
    if(isAndroid){location.href=externalBrowserIntentV1133();return}
    try{await navigator.clipboard.writeText(target.href);window.baemoonApp?.toast?.('주소를 복사했습니다. Safari에서 붙여넣어 열어주세요.')}catch{window.open(target.href,'_blank','noopener,noreferrer')}
  }
  async function kakaoQrNoticeV1133(){
    stopCameraTracksV1133();
    const confirm=window.__bmAppConfirmV1130;
    const open=confirm?await confirm('카카오톡에서는 QR 카메라를 열지 않습니다.','인앱 브라우저 충돌을 막기 위해 Chrome 또는 기본 브라우저에서 QR을 확인합니다.','외부 브라우저로 열기'):window.confirm('카카오톡에서는 QR 카메라가 불안정합니다. 외부 브라우저로 열까요?');
    if(open)openExternalV1133();
  }
  function markKakaoUiV1133(){
    if(!isKakao)return;
    const start=document.querySelector('#startQrScannerButton');
    if(start){start.textContent='외부 브라우저에서 QR 스캔';start.dataset.kakaoExternal='1'}
    const modal=document.querySelector('#qrScannerModal .modal-card');
    if(modal&&!modal.querySelector('.kakao-qr-note-v1133')){
      const note=document.createElement('div');note.className='kakao-qr-note-v1133';
      note.innerHTML='<b>카카오톡 안에서는 카메라 스캔을 사용하지 않습니다.</b><br>QR은 Chrome 또는 기본 브라우저에서 열어주세요.<button type="button">외부 브라우저로 열기</button>';
      note.querySelector('button').addEventListener('click',openExternalV1133);
      modal.querySelector('.modal-subtitle')?.insertAdjacentElement('afterend',note);
    }
  }
  document.addEventListener('click',event=>{
    const start=event.target.closest('#startQrScannerButton');
    if(start&&isKakao){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();kakaoQrNoticeV1133();return}
    if(event.target.closest('#openQrScannerButton'))setTimeout(markKakaoUiV1133,0);
  },true);
  window.addEventListener('pagehide',stopCameraTracksV1133);
  window.addEventListener('beforeunload',stopCameraTracksV1133);
  document.addEventListener('visibilitychange',()=>{if(document.hidden)stopCameraTracksV1133()});

  function normalizeReservationV1133(row={}){return {id:String(row.id||''),slotId:String(row.slot_key||row.slotId||''),festivalId:String(row.festival_id||row.festivalId||''),festivalName:String(row.festival_name||row.festivalName||''),boothId:String(row.booth_id||row.boothId||''),boothName:String(row.booth_name||row.boothName||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||row.userUid||''),participantRole:String(row.participant_role||row.participantRole||''),user:String(row.display_name||row.displayName||row.user||'예약자'),name:String(row.display_name||row.displayName||row.name||'예약자'),guestCode:String(row.guest_code||row.guestCode||''),groupSize:Number(row.group_size||row.groupSize||1),status:'confirmed',checkInCode:String(row.check_in_code||row.checkInCode||''),checkedIn:false,checkedInAt:null,createdAt:Number(row.created_at||row.createdAt||Date.now()),updatedAt:Number(row.updated_at||row.updatedAt||Date.now())}}
  function normalizeQueueV1133(row={}){return {id:String(row.id||''),slotId:String(row.slot_key||row.slotId||''),festivalId:String(row.festival_id||row.festivalId||''),festivalName:String(row.festival_name||row.festivalName||''),boothId:String(row.booth_id||row.boothId||''),boothName:String(row.booth_name||row.boothName||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||row.userUid||''),participantRole:String(row.participant_role||row.participantRole||''),displayName:String(row.display_name||row.displayName||'예약자'),guestCode:String(row.guest_code||row.guestCode||''),queueNumber:Number(row.queue_number||row.queueNumber||0),status:String(row.status||'waiting')==='called'?'called':'waiting',createdAt:Number(row.created_at||row.createdAt||Date.now()),calledAt:Number(row.called_at||row.calledAt||0)||null,aheadCount:Number(row.aheadCount||0),callCount:Number(row.call_count||row.callCount||0)}}
  async function hardSyncMineV1133(){
    const session=window.baemoonApp?.session?.();
    if(!session?.uid||!['student','guest'].includes(session.role)||syncBusy||!window.baemoonFirebase?.getMyServerOverview)return;
    syncBusy=true;
    try{
      const payload=await window.baemoonFirebase.getMyServerOverview();
      const reservations=(payload.reservations||[]).map(normalizeReservationV1133);
      const queue=(payload.queue||[]).map(normalizeQueueV1133);
      ...
      window.baemoonApp?.renderHomeReservations?.();
      window.baemoonApp?.renderMy?.();
    }catch(error){console.warn('v11.33 server reconciliation failed',error)}finally{syncBusy=false}
  }
  async function waitRuntimeV1133(){for(let i=0;i<180;i++){if(window.baemoonFirebase&&window.baemoonApp&&window.__firebaseRuntimeReady)return true;await sleep(100)}return false}
  async function startReconcileV1133(){
    if(!await waitRuntimeV1133())return;
    await hardSyncMineV1133();
    setTimeout(hardSyncMineV1133,1200);setTimeout(hardSyncMineV1133,4500);
    markKakaoUiV1133();
  }
  window.addEventListener('pageshow',()=>startReconcileV1133());
  window.addEventListener('focus',hardSyncMineV1133);
  window.addEventListener('online',hardSyncMineV1133);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)hardSyncMineV1133()});
  window.addEventListener('baemoon:app-ready',()=>startReconcileV1133(),{once:true});
  setTimeout(startReconcileV1133,0);
  setInterval(()=>{if(!document.hidden)hardSyncMineV1133()},8000);

  function ensureParticipationDetailV1133(){
    let overlay=document.querySelector('#participationDetailV1133');if(overlay)return overlay;
    overlay=document.createElement('div');overlay.id='participationDetailV1133';overlay.className='participation-detail-v1133';overlay.hidden=true;
    overlay.innerHTML='<section role="dialog" aria-modal="true"><div class="participation-detail-head-v1133"><div><h2 id="participationDetailTitleV1133">참여자 명단</h2><p id="participationDetailSummaryV1133"></p></div><button class="participation-detail-close-v1133" type="button" aria-label="닫기">×</button></div><div class="participation-detail-list-v1133" id="participationDetailListV1133"></div></section>';
    overlay.querySelector('.participation-detail-close-v1133').addEventListener('click',()=>overlay.hidden=true);
    overlay.addEventListener('click',event=>{if(event.target===overlay)overlay.hidden=true});document.body.append(overlay);return overlay;
  }
  async function openParticipationDetailV1133(article){
    const booth=article.querySelector('b')?.textContent?.trim()||'부스';
    const festival=(article.querySelector('small')?.textContent||'').split('·')[0].trim();
    const overlay=ensureParticipationDetailV1133(),list=overlay.querySelector('#participationDetailListV1133');
    overlay.querySelector('#participationDetailTitleV1133').textContent=booth+' 참여자';overlay.querySelector('#participationDetailSummaryV1133').textContent=festival;
    list.innerHTML='<div class="admin-empty">불러오는 중…</div>';overlay.hidden=false;
    try{
      const payload=await window.baemoonFirebase.getParticipations({limit:1000});
      const rows=(payload.participations||[]).filter(row=>String(row.booth_name||'')===booth&&(!festival||String(row.festival_name||'')===festival)).sort((a,b)=>Number(b.checked_in_at||0)-Number(a.checked_in_at||0));
      const people=rows.reduce((sum,row)=>sum+Math.max(1,Number(row.group_size||1)),0);
      overlay.querySelector('#participationDetailSummaryV1133').textContent=`${festival||'행사'} · ${rows.length}팀 · ${people}명`;
      list.innerHTML=rows.length?rows.map(row=>`<article><div><b>${esc(row.display_name||'참여자')}</b><p>${row.participant_role==='guest'?`게스트${row.guest_code?` · 확인번호 ${esc(row.guest_code)}`:''}`:'학생'} · ${esc(row.time||'즉시 예약')}<br>${new Date(Number(row.checked_in_at||Date.now())).toLocaleString('ko-KR')}</p></div><strong>${Math.max(1,Number(row.group_size||1))}명</strong></article>`).join(''):'<div class="admin-empty">참여 기록이 없습니다.</div>';
    }catch(error){list.innerHTML=`<div class="admin-empty">${esc(error.message||'참여자 명단을 불러오지 못했습니다.')}</div>`}
  }
  document.addEventListener('click',event=>{
    const article=event.target.closest('#adminParticipationHistoryV1129 article');if(!article)return;
    event.preventDefault();openParticipationDetailV1133(article);
  });
  const observer=new MutationObserver(()=>{
    document.querySelectorAll('#adminParticipationHistoryV1129 article').forEach(article=>{
      if(article.querySelector('.participation-open-hint-v1133'))return;
      const hint=document.createElement('span');hint.className='participation-open-hint-v1133';hint.textContent='눌러서 참여자 명단 보기';article.querySelector('div')?.append(hint);
    });
    markKakaoUiV1133();
  });
