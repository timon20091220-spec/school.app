...
(()=>{
  ...
  function ui1202(){
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.01)/g,'v12.05'));
    const logoutText=document.querySelector('#logoutButton b');if(logoutText)logoutText.textContent='로그아웃';
...
(()=>{
  ...
  function ui1203(){
    document.documentElement.dataset.baemoonVersion='13.00';
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.0[12])/g,'v12.05'));
  }
...
(()=>{
  ...
  window.addEventListener('change',event=>{if(!event.target?.matches?.('#notificationToggle'))return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();event.target.checked?enablePush({silent:false}):disablePush()},true);
  ...
  const observer=new MutationObserver(()=>{clearTimeout(uiTimer);uiTimer=setTimeout(runUi,30)});observer.observe(document.body,{childList:true,subtree:true});
...
(()=>{
  ...
  window.addEventListener('change',event=>{if(!event.target?.matches?.('#notificationToggle'))return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();window.__handleNotificationToggleV1205(event.target.checked)},true);

  async function foregroundPoll(){const session=window.baemoonApp?.session?.();if(pollBusy||document.hidden||!session?.uid||!['student','guest'].includes(session.role))return;pollBusy=true;try{const data=await request('/api/me/overview');const next=new Map();for(const row of data.queue||[]){const id=String(row.id||''),status=String(row.status||'waiting'),count=Number(row.call_count||row.callCount||0);next.set(id,{status,count});const before=lastQueueState.get(id);if(before&&before.status!=='called'&&status==='called'){const body=`${row.booth_name||row.boothName||'부스'} 부스에서 호출입니다. QR을 준비하고 부스로 이동해주세요.`;window.baemoonApp?.toast?.(body);try{const reg=await registration();reg.active?.postMessage({type:'SHOW_NOTIFICATION',title:'대기번호가 호출되었습니다',body,type:'queue-call',queueId:id,messageId:`foreground-${id}-${count}-${Date.now()}`,url:'./?open=queue'})}catch{}}}lastQueueState=next}catch{}finally{pollBusy=false}}
  ...
  function removeLegacyUi(){document.querySelectorAll('#festivalAdminQuickToggleV1203,.festival-admin-quick-toggle-v1203,#testNotificationV1202,#diagnoseNotificationV1202,#notificationDiagnosticV1202,#testBackgroundPushV1143').forEach(n=>n.remove());document.documentElement.dataset.baemoonVersion='13.00'}
  function tick(){removeLegacyUi();authAndPushTick();foregroundPoll();clearTimeout(timer);timer=setTimeout(tick,4000)}
  const observer=new MutationObserver(()=>{removeLegacyUi();updateHint()});observer.observe(document.body,{childList:true,subtree:true});
...
/* ===== 배문고 앱 v13.00 안정화 레이어 ===== */
(()=>{
  'use strict';
  if(window.__BAEMOON_V1300__)return;
  window.__BAEMOON_V1300__=true;
  const VERSION='13.00.1';
  ...
  function normalizeGuestForm(){
    ...
    if(old){
      const legacy=old.querySelector('#guestFirstPasswordV1205');if(legacy)legacy.remove();
      const label=old.querySelector('.guest-password-field-v1205 span');if(label&&label.textContent!=='비밀번호')label.textContent='비밀번호';
      const input=old.querySelector('#guestPasswordV1205');if(input&&input.placeholder!=='최초 이용자는 baemoon2026')input.placeholder='최초 이용자는 baemoon2026';
      const notice=old.querySelector('#guestAccountRuleV1205'),noticeText='처음 이용할 때는 공통 비밀번호 baemoon2026을 입력한 뒤 개인 비밀번호를 설정합니다.';
      if(notice&&notice.textContent!==noticeText)notice.textContent=noticeText;
    }
    const button=card.querySelector('#confirmGuestEntry');if(button&&button.textContent!=='게스트 로그인')button.textContent='게스트 로그인';
  }
  ...
  async function stablePushToggle(enabled,{ask=true,toast=true}={}){
    ...
    try{
      ...
      if(!enabled){
        ...
        localStorage.setItem(NOTIFY_KEY,'0');localStorage.setItem('bm_notify_default_v1202','0');if(toggle)toggle.checked=false;
      ...
      localStorage.setItem(NOTIFY_KEY,'1');localStorage.setItem('bm_notify_default_v1202','1');if(toggle){toggle.checked=true;toggle.dataset.pushReady='1'}
      if(toast)window.baemoonApp?.toast?.('앱 알림을 켰습니다.');return true;
  ...
  function keepVersion(){
    if(document.documentElement.dataset.baemoonVersion!==VERSION)document.documentElement.dataset.baemoonVersion=VERSION;
    document.querySelectorAll('[data-app-version],.app-version,.auth-welcome-card small').forEach(element=>{
      const current=element.textContent||'';
      if(!/v(?:11|12)\.\d+/i.test(current))return;
      const next=current.replace(/v(?:11|12)\.\d+/ig,'v13.00.1');
  ...
  window.addEventListener('online',()=>setTimeout(autoReconnect,250));
  setInterval(scheduleTick,3000);
  setTimeout(scheduleTick,0);
  console.info('Baemoon v13.00.1 stabilization hotfix ready');
})();
