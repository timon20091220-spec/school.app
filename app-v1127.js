...
(()=>{
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      const expected=time==='즉시 예약'?'즉시 참여 예정':`${time} 참여 예정`;
...
/* ===== v12.01 festival guide · mobile push guidance · instant check-in · UI corrections ===== */
(()=>{
  'use strict';
  const VERSION='12.01';
  ...
  function setVersionV1201(){
    ...
    document.querySelectorAll('[data-app-version],.app-version,.firebase-beta-status+small,.auth-welcome-card>small').forEach(element=>{
      ...
      if(/v11\.(?:27|33|34|35|36|42|43|44)/.test(text))element.textContent=text.replace(/v11\.(?:27|33|34|35|36|42|43|44)/g,'v12.01');
    });
  }
  ...
  async function refreshAfterPushV1201(payload={}){
    ...
    try{
      ...
      if(payload.type==='check-in'){
        try{navigator.vibrate?.([260,100,260])}catch{}
        if(window.__bmAppAlertV1130)await window.__bmAppAlertV1130('체크인이 완료되었습니다',payload.body||'부스 참여가 완료되었습니다.');else toast(payload.body||'체크인이 완료되었습니다.');
      }else if(payload.type==='queue-call'){
        try{navigator.vibrate?.([500,160,500,160,700])}catch{}
        if(!document.hidden&&window.__bmAppAlertV1130)await window.__bmAppAlertV1130('대기번호가 호출되었습니다',payload.body||'QR을 준비하고 부스로 이동해주세요.');
      }
    }catch(error){console.warn('v12.01 push refresh failed',error)}
  }
  ...
  (async()=>{
    ...
    console.info('Baemoon v12.01 features ready');
  })();
})();

/* v12.01 active QR check-in confirmation fallback: only polls while the student's QR is open. */
(()=>{
  ...
  setInterval(async()=>{
    ...
    try{
      ...
      window.baemoonApp?.renderHomeReservations?.();window.baemoonApp?.renderMy?.();
      if(lastShown!==key){lastShown=key;try{navigator.vibrate?.([260,100,260])}catch{};if(window.__bmAppAlertV1130)await window.__bmAppAlertV1130('체크인이 완료되었습니다',`${participation.booth_name||participation.boothName||'부스'} 참여가 확인되었습니다.`)}
