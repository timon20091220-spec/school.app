...
(()=>{
  ...
  function keepVersion(){
    document.documentElement.dataset.baemoonVersion=VERSION;
    document.querySelectorAll('[data-app-version],.app-version,.auth-welcome-card small').forEach(element=>{const text=element.textContent||'';if(/v(?:11|12)\.\d+/i.test(text))element.textContent=text.replace(/v(?:11|12)\.\d+/ig,'v13.00')});
  }
  ...
  const observer=new MutationObserver(tick);observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('pageshow',tick);window.addEventListener('online',()=>setTimeout(autoReconnect,250));setInterval(tick,3000);setTimeout(tick,0);
  console.info('Baemoon v13.00 stabilization layer ready');
})();
