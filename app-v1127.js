...
(()=>{
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      const ahead=Math.max(0,number-1),duration=Math.max(1,Number(booth.duration||15)),waitMinutes=ahead*duration,expectedClock=new Date(Date.now()+waitMinutes*60000).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'});const expected=time==='즉시 예약'?(ahead===0?'곧 참여 예정':`${expectedClock} 전후 예상 · 앞 ${ahead}팀`):`${time} 참여 예정`;
...
(()=>{
  ...
  function setVersionV1201(){
    ...
    document.querySelectorAll('[data-app-version],.app-version,.firebase-beta-status+small,.auth-welcome-card>small').forEach(element=>{
      ...
      if(/v11\.(?:27|33|34|35|36|42|43|44)/.test(text))element.textContent=text.replace(/v11\.(?:27|33|34|35|36|42|43|44)/g,'v12.02');
    });
  }
  ...
  (async()=>{
    ...
    console.info('Baemoon v12.02 features ready');
  })();
})();
...
(()=>{
  ...
  function integrateNotifications1202(){
    ...
    let controls=row.parentElement?.querySelector('#notificationToolsV1202');
    if(!controls){controls=document.createElement('div');controls.id='notificationToolsV1202';controls.className='notification-tools-v1202';controls.innerHTML='<button type="button" id="testNotificationV1202">테스트 알림</button><button type="button" id="diagnoseNotificationV1202">알림 상태 확인</button><div id="notificationDiagnosticV1202" hidden></div>';row.insertAdjacentElement('afterend',controls)}
  ...
  async function toggleNotifications1202(enabled){
    ...
    try{
      if(enabled){await saveCurrentPush1202({requestPermission:true});window.baemoonApp?.toast?.('앱 알림을 켰습니다. 사이트를 닫아도 지원되는 브라우저에서는 호출 알림이 도착합니다.')}
      else{await deactivatePush1202();await window.baemoonFirebase?.setNotificationPreference?.(false);window.baemoonApp?.toast?.('앱 알림을 껐습니다.')}
      if(toggle)toggle.checked=enabled;
    }catch(error){if(toggle)toggle.checked=false;window.baemoonApp?.toast?.(error.message||'알림 설정을 변경하지 못했습니다.')}finally{if(toggle)toggle.disabled=false}
  }
  ...
  function ui1202(){
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.01)/g,'v12.02'));
    const logoutText=document.querySelector('#logoutButton b');if(logoutText)logoutText.textContent='로그아웃';
  ...
  (async()=>{
    ...
    console.info('Baemoon v12.02 features ready');
  })().catch(error=>console.error('v12.02 initialization failed',error));
})();
