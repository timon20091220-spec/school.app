...
if('serviceWorker' in navigator){
  navigator.serviceWorker.addEventListener('controllerchange',()=>{
    // v11.29: 새 서비스 워커가 설치되어도 사용 중인 화면을 강제로 새로고침하지 않습니다.
...
async function showDeviceNotification(item){
  ...
  if(navigator.vibrate)try{navigator.vibrate([350,120,350,120,700])}catch{}
...
document.addEventListener('click',async event=>{
  ...
  const payload={
    ...
    openStart,
    openEnd,
    bookingClosed:$('#boothBookingStatus').value==='closed',
    congestion:$('#boothCongestion').value||'auto',
...
/* ===== v11.27 행사 운영 기능 ===== */
function queueEntries(){return window.firebaseCache?.queueEntries||[]}
...
function boothStatusInfo(festival,booth){
  ...
  if(congestion==='auto'){
    congestion=waiting>0||maxRatio>=.8?'crowded':maxRatio>=.4?'normal':'relaxed';
  }
...
renderMy=function(){
  ...
  $('#myQueueList').innerHTML=mine.map(item=>{
    ...
    return `<article class="my-queue-card ${item.status}">
      <div><span>${item.status==='called'?'호출됨':`대기 ${position||'-'}번`}</span>
...
async function processQrCheckIn(code){
  const result=$('#qrCheckInResult');
  result.textContent='확인 중…';
  try{
    const checked=await window.baemoonFirebase.checkInReservationCode(String(code||'').trim());
    result.innerHTML=`<b>체크인 완료</b><span>${esc(checked.user||'예약자')} · ${esc(checked.boothName||'부스')} · ${Number(checked.groupSize||1)}명</span>`;
    toast('예약 체크인을 완료했습니다.');
    renderAdminStats();
    renderAdminReservations();
    return true;
  }catch(error){
    result.textContent=firebaseFriendlyMessage(error);
    return false;
  }
}
...
function renderReservationServerStatus(){
  ...
  const map={
    checking:['예약 서버 확인 중','Cloudflare 연결을 확인합니다.'],
    syncing:['예약 서버 동기화 중','행사 부스와 회차를 서버에 반영하고 있습니다.'],
    connected:['예약 서버 연결됨','동시 예약 순차 처리 서버가 정상 작동 중입니다.'],
...
function reservationSlotPast(time){
  const value=parseAppDateTime(time);
...
openReservation=function(boothId){
  ...
  buttons.forEach(button=>{
    if(reservationSlotPast(button.dataset.time)){
      button.disabled=true;
...
(()=>{
  ...
  function normalizeQueue(row={}){
    return {
      ...
      status:String(row.status||'waiting'),createdAt:Number(row.created_at||row.createdAt||Date.now()),calledAt:Number(row.called_at||row.calledAt||0)||null,checkedInAt:Number(row.checked_in_at||row.checkedInAt||0)||null,
  ...
  function installApiExtensions(){
    ...
    api.callNextQueue=async payload=>{
      ...
      if(result?.queue&&!result?.alreadyCalled)await notifyQueueCall(result.queue);
      return result;
    };
    ...
    api.checkInReservationCode=async rawCode=>{
      ...
      window.firebaseCache.reservations=(window.firebaseCache.reservations||[]).filter(item=>item.id!==reservation.id);
      if(result.queue?.id)window.firebaseCache.queueEntries=(window.firebaseCache.queueEntries||[]).filter(item=>item.id!==String(result.queue.id));
  ...
  async function deleteBoothServerFirst(button){
    ...
    try{
      ...
      const message=`${booth.name} 부스를 삭제하면 현재 활성 예약 ${Number(preview.reservations||0)}건, 번호 ${Number(preview.queueEntries||0)}건, 예약 시간 ${Number(preview.slots||0)}개도 함께 정리됩니다. 참여 완료 기록은 통계에 남깁니다. 계속할까요?`;
      ...
      appToast(`부스 삭제 완료 · 예약 ${Number(deleted.deletedReservations||0)}건 · 번호 ${Number(deleted.deletedQueueEntries||0)}건 정리`);
  ...
  function ensureCapacityHelp(){
    ...
    if(!label.querySelector('.capacity-help-v1129')){const small=document.createElement('small');small.className='capacity-help-v1129';small.textContent='새 시간을 추가할 때 사용할 기본값입니다. 아래 시간 목록의 “이 시간 정원”에서 10시 5명, 11시 10명처럼 각각 따로 바꿀 수 있습니다.';label.append(small)}
  }
  function ensureQueueGuide(){
    ...
    const guide=document.createElement('div');guide.className='queue-guide-v1129';guide.innerHTML='<b>예약하면 바로 번호가 발급됩니다</b><span>정원이 남아 있는 예약만 접수되며, 시간제 부스는 시간마다 1번부터, 시간이 없는 부스는 “즉시 예약” 번호로 발급됩니다. 관리자가 호출한 뒤 QR을 보여주면 입장이 완료됩니다.</span>';panel.prepend(guide);
  }
  function ensureAdminUtilityButtons(){
    const sync=document.querySelector('#syncReservationServerButton');if(!sync)return;
    if(!document.querySelector('#cleanupOrphanReservationsV1129')){
      const button=document.createElement('button');button.id='cleanupOrphanReservationsV1129';button.type='button';button.textContent='삭제된 부스의 남은 예약 정리';sync.insertAdjacentElement('afterend',button);
      button.addEventListener('click',async()=>{
        const valid=festivals().flatMap(festival=>(festival.booths||[]).map(booth=>({festivalId:String(festival.id),boothId:String(booth.id)})));
        if(!confirm('예전에 화면에서는 삭제됐지만 예약 서버에만 남아 있는 부스의 활성 예약·번호·시간 데이터를 찾습니다. 현재 존재하는 부스 데이터와 참여 완료 기록은 건드리지 않습니다. 계속할까요?'))return;
        button.disabled=true;const old=button.textContent;button.textContent='정리 중…';
        try{const result=await window.baemoonFirebase.purgeOrphanedBooths(valid);appToast(`남은 부스 ${Number(result.deletedBooths||0)}개 · 예약 ${Number(result.deletedReservations||0)}건을 정리했습니다.`);await window.baemoonFirebase.getAdminStats({force:true})}catch(error){appToast(error.message||'정리에 실패했습니다.')}finally{button.disabled=false;button.textContent=old}
      });
    }
    if(!document.querySelector('#resetTestReservationsV1129')){
      const button=document.createElement('button');button.id='resetTestReservationsV1129';button.type='button';button.textContent='테스트 예약 기록 초기화';document.querySelector('#cleanupOrphanReservationsV1129')?.insertAdjacentElement('afterend',button);
      button.addEventListener('click',async()=>{
        const phrase=prompt('활성 예약과 번호를 모두 초기화합니다. Firebase 학생 계정은 삭제하지 않습니다. 계속하려면 “배문고 테스트 초기화”를 입력하세요.');
        if(phrase!=='배문고 테스트 초기화')return appToast('확인 문구가 달라 초기화를 취소했습니다.');
        const includeParticipations=confirm('체크인 완료된 참여 현황도 함께 지울까요?\n확인: 참여 현황까지 삭제\n취소: 참여 현황은 보존');
        button.disabled=true;const old=button.textContent;button.textContent='초기화 중…';
        try{const result=await window.baemoonFirebase.resetTestReservationData({confirmation:phrase,includeParticipations});await window.baemoonFirebase.getAdminStats({force:true});appToast(`예약 ${Number(result.deletedReservations||0)}건 · 번호 ${Number(result.deletedQueueEntries||0)}건을 초기화했습니다.`)}catch(error){appToast(error.message||'초기화에 실패했습니다.')}finally{button.disabled=false;button.textContent=old}
      });
    }
  }

  function ensureQueueTimeSelector(){
    const entry=selectedAdminEntry(),list=document.querySelector('#adminBoothQueueList');if(!entry||!list)return;
    let wrap=document.querySelector('#adminQueueTimeWrapV1129');
    if(!wrap){wrap=document.createElement('label');wrap.id='adminQueueTimeWrapV1129';wrap.className='admin-queue-time-v1129';wrap.innerHTML='<span>관리할 예약 시간</span><select id="adminQueueTimeV1129"></select>';list.parentElement?.insertBefore(wrap,list);wrap.querySelector('select').addEventListener('change',event=>loadQueueTime(event.target.value))}
    const select=wrap.querySelector('select'),previous=select.value;select.innerHTML=boothTimes(entry.booth).map(time=>`<option value="${htmlEsc(time)}">${htmlEsc(time==='즉시 예약'?'즉시 예약':String(time).replace('T',' '))}</option>`).join('');if([...select.options].some(option=>option.value===previous))select.value=previous;
  }
  ...
  function enhanceQueueRows(){
    ...
    document.querySelectorAll('#adminBoothQueueList article').forEach((article,index)=>{
      ...
      const span=article.querySelector('span');if(span)span.textContent=item.status==='called'?`호출됨 · ${item.queueNumber}번`:`${item.queueNumber}번`;
      const rowCall=article.querySelector('[data-call-queue]');if(rowCall)rowCall.hidden=true;
  ...
  async function enhanceMyQueue(force=false){
    ...
    try{
      ...
      cards.forEach((card,index)=>{
        ...
        if(span)span.textContent=item.status==='called'?`호출됨 · ${Number(item.queueNumber||0)}번 · ${time}`:`번호 ${Number(item.queueNumber||0)}번 · 앞에 ${Number(item.aheadCount||0)}팀 · ${time}`;
      });
  ...
  async function renderParticipationPanel(force=false){
    ...
    if(!section){section=document.createElement('section');section.id='adminParticipationHistoryV1129';section.className='admin-participation-v1129';section.innerHTML='<div class="admin-participation-head"><div><b>부스 참여 현황</b><small>QR 체크인을 완료한 기록입니다. 게스트는 이름과 기기별 확인번호를 함께 표시합니다.</small></div><button type="button" id="refreshParticipationsV1129">새로고침</button></div><div id="participationListV1129" class="participation-list-v1129"><div class="admin-empty">불러오는 중…</div></div>';ranking.parentElement?.insertAdjacentElement('afterend',section);section.querySelector('#refreshParticipationsV1129').addEventListener('click',()=>renderParticipationPanel(true))}
  ...
  function ensureQrGuides(){
    const student=document.querySelector('#reservationQrModal .modal-card');if(student&&!student.querySelector('.qr-guide-v1129')){const p=document.createElement('p');p.className='qr-guide-v1129';p.textContent='내 번호가 호출된 뒤 부스 담당 관리자에게 이 QR을 보여주세요. 스캔이 완료되면 활성 예약 목록에서는 사라지고 참여 현황에 기록됩니다.';student.querySelector('h2')?.insertAdjacentElement('afterend',p)}
    const admin=document.querySelector('#qrScannerModal .modal-card');if(admin&&!admin.querySelector('.qr-guide-v1129')){const p=document.createElement('p');p.className='qr-guide-v1129';p.textContent='먼저 해당 시간의 번호를 호출한 뒤 QR을 스캔하세요. 호출되지 않은 번호는 체크인되지 않습니다.';admin.querySelector('h2')?.insertAdjacentElement('afterend',p)}
  }
  ...
  function patchVersionLabels(){
    ...
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.29')}
  ...
  function observeUi(){
    ...
    const observer=new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(run)});observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','hidden','data-action']});run();
  }

  (async()=>{
    try{await waitForRuntime();installApiExtensions();installBannerBehavior();observeUi();console.info(`Baemoon school app v${VERSION} ready`)}catch(error){console.error('v11.29 initialization failed:',error)}
  })();
})();
