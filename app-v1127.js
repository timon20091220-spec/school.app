...
(()=>{
  ...
  function patchVersionLabels(){
    ...
    let node;while((node=walker.nextNode())){if(node.nodeValue?.trim()==='v11.27')node.nodeValue=node.nodeValue.replace('v11.27','v11.36')}
...
(()=>{
  ...
  function scheduleSync(delay){clearTimeout(syncTimer);syncTimer=setTimeout(async()=>{if(!document.hidden)await syncMine();const fast=document.querySelector('#reservationDetailModal.open,#reservationQrModal.open')||window.baemoonApp?.state?.screen==='my';scheduleSync(fast?1500:30000)},delay)}
  ...
  function manualQrOnly(){
    ...
    if(isKakao){const subtitle=document.querySelector('#qrScannerModal .modal-subtitle');if(subtitle)subtitle.textContent='카카오톡에서는 QR 아래의 코드를 직접 입력하거나 Chrome에서 스캔하세요.'}
  }

  async function startSafeScanner(){
    stopCamera();manualQrOnly();
    if(isKakao){await (window.__bmAppAlertV1130?window.__bmAppAlertV1130('카카오톡에서는 카메라 스캔을 사용하지 않습니다.','사이트는 그대로 이용할 수 있습니다. QR 아래의 코드를 직접 입력하거나 Chrome에서 열어주세요.'):Promise.resolve());return}
    ...
    }catch(error){stopCamera();document.querySelector('.manual-qr-fallback-v1134')?.setAttribute('open','');toast('카메라를 열 수 없습니다. 코드를 직접 입력해주세요.')}
  }
...
(()=>{
  ...
  const observer=new MutationObserver(()=>{applyFestivalWordsV1135();updateResetWordsV1135()});
  observer.observe(document.body,{childList:true,subtree:true});updateResetWordsV1135();
  document.documentElement.dataset.baemoonVersion=VERSION;
...
(()=>{
  ...
  const observer=new MutationObserver(()=>{
    applyFestivalHubAdminMode();prepareContinuousFestival();enhanceHomeFestivalCards();ensureParticipationDeleteButton();
  ...
  (async()=>{
    ...
    console.info('Baemoon v11.36 ready');
  })();
  document.documentElement.dataset.baemoonVersion=VERSION;
})();
