...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1115.js',{scope:'./',updateViaCache:'none'})
...
function waitForInstallPrompt(timeout=10000){
...
function showInstallFallback(){
  ...
  $('#installHelpText').textContent=isIOS||isSafari
    ?'Safari의 공유 버튼을 누른 뒤 ‘홈 화면에 추가’를 선택하면 앱으로 설치됩니다.'
    :'브라우저 주소창 또는 메뉴의 ‘앱 설치’·‘홈 화면에 추가’를 선택해주세요.';
  ...
  $('#homeReservationList').innerHTML=mine.length
    ?mine.map(item=>{
      ...
      return `<button class="home-reservation-card" data-open-reservation="${item.id}">
        ...
        <div><b>${esc(info.boothName)}</b><small>${esc(info.festivalName)} · ${esc(info.location)}</small></div>
        <i>상세 보기</i>
      </button>`;
    }).join('')
  ...
  $('#myReservationList').innerHTML=mine.length
    ?mine.sort((a,b)=>Number(a.createdAt)-Number(b.createdAt)).map(item=>{
      ...
      return `<button class="my-reservation-card" data-open-reservation="${item.id}">
        ...
        <p>${esc(info.time)} · ${esc(info.location)}</p>
...
function openReservationMessage(reservationId){
  ...
  const info=resolvedReservationInfo(reservation);
  $('#reservationMessageTarget').textContent=`${info.user} · ${info.boothName} · ${info.time}`;
...
function renderAdminReservations(){
  ...
  $('#adminReservationList').innerHTML=items.length
    ?items.map(item=>{
      const info=resolvedReservationInfo(item);
      return `<div class="admin-row admin-reservation-row">
        <div><b>${esc(info.festivalName)} · ${esc(info.boothName)}</b><small>${esc(info.user)} · ${esc(info.location)}</small></div>
        <span>${esc(info.time)}</span>
        <small>${new Date(info.createdAt).toLocaleString('ko-KR')}</small>
