...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],queueEntries:[],notifications:[],
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.25 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.25 Firestore 규칙을 게시해주세요.';
...
function startDataListeners(role,profile=null){
  ...
  listenQueueEntries(role,auth.currentUser.uid);
...
window.baemoonFirebase={
  ...
  async createReservation(payload){
    ...
    const festivalRef=doc(db,'festivals',payload.festivalId);

    try{
      return await runTransaction(db,async transaction=>{
        const festivalSnap=await transaction.get(festivalRef);
        const reservationSnap=await transaction.get(reservationRef);
        const slotSnap=await transaction.get(slotRef);

        if(!festivalSnap.exists())throw new Error('행사 정보를 찾지 못했습니다.');
        const festival=festivalSnap.data();
        const booth=(festival.booths||[]).find(item=>item.id===payload.boothId);
        if(!booth)throw new Error('부스 정보를 찾지 못했습니다.');
        if(booth.bookingClosed)throw new Error('현재 예약이 마감된 부스입니다.');
        ...
        const data={
          ...
          minPeople,
          capacity,
          checkInCode:existing?.checkInCode||reservationCheckInCode(),
          checkedIn:Boolean(existing?.checkedIn),
  ...
  async joinBoothQueue(payload){
    ...
    const current=ui().session();
    if(!['student','guest'].includes(current?.role)){
      throw new Error('학생 또는 게스트만 대기번호를 받을 수 있습니다.');
    }

    const festivalSnap=await getDoc(doc(db,'festivals',String(payload.festivalId)));
    if(!festivalSnap.exists())throw new Error('행사 정보를 찾지 못했습니다.');
    const booth=(festivalSnap.data().booths||[]).find(item=>item.id===payload.boothId);
    if(!booth)throw new Error('부스 정보를 찾지 못했습니다.');
    if(!booth.waitlistEnabled)throw new Error('이 부스는 현장 대기번호를 사용하지 않습니다.');

    const entryId=`${payload.festivalId}_${payload.boothId}_${auth.currentUser.uid}`;
    const entryRef=doc(db,'queueEntries',entryId);
    const existing=await getDoc(entryRef);
    if(existing.exists()&&['waiting','called'].includes(existing.data().status)){
    ...
    const data={
      ...
      festivalName:String(payload.festivalName||'행사'),
      boothId:String(payload.boothId),
      boothName:String(payload.boothName||'부스'),
      userUid:auth.currentUser.uid,
      participantRole:current.role,
      status:'waiting',
      createdAt:Date.now(),
      updatedAt:Date.now()
    };
    await setDoc(entryRef,data,{merge:false});
    return data;
  },
  async cancelQueueEntry(entryId){
    requireUser();
    const ref=doc(db,'queueEntries',String(entryId));
    const snap=await getDoc(ref);
    if(!snap.exists())return false;
    const data=snap.data();
    if(!isAdminUser()&&data.userUid!==auth.currentUser.uid){
      throw new Error('본인의 대기번호만 취소할 수 있습니다.');
    }
    await updateDoc(ref,{status:'cancelled',updatedAt:Date.now()});
    return true;
  },
  async updateQueueStatus(entryId,status){
    requireAdmin();
    const allowed=['waiting','called','checkedin','cancelled'];
    if(!allowed.includes(status))throw new Error('올바르지 않은 대기 상태입니다.');
    const ref=doc(db,'queueEntries',String(entryId));
    await updateDoc(ref,{
      status,
      calledAt:status==='called'?Date.now():null,
      checkedInAt:status==='checkedin'?Date.now():null,
      updatedAt:Date.now()
    });
    return true;
  },
  async ensureReservationCheckInCode(reservationId){
    ...
    const snap=await getDoc(ref);
    if(!snap.exists())throw new Error('예약 정보를 찾지 못했습니다.');
    const data=snap.data();
    if(!isAdminUser()&&data.userUid!==auth.currentUser.uid){
      throw new Error('본인의 예약만 확인할 수 있습니다.');
    }
    if(data.checkInCode)return {id:snap.id,...data};
    const checkInCode=reservationCheckInCode();
    await updateDoc(ref,{checkInCode,checkedIn:Boolean(data.checkedIn),updatedAt:Date.now()});
    return {id:snap.id,...data,checkInCode};
  },
  async checkInReservationCode(rawCode){
    requireAdmin();
    const parts=String(rawCode||'').trim().split('|');
    if(parts.length!==3||parts[0]!=='BMRES'){
      throw new Error('올바른 체크인 코드가 아닙니다.');
    }
    const [,reservationId,token]=parts;
    const ref=doc(db,'reservations',reservationId);
    const snap=await getDoc(ref);
    if(!snap.exists())throw new Error('예약 정보를 찾지 못했습니다.');
    const data=snap.data();
    if(String(data.checkInCode||'')!==token){
      throw new Error('체크인 확인 코드가 일치하지 않습니다.');
    }
    if(data.checkedIn)return {id:snap.id,...data};
    const checked={checkedIn:true,checkedInAt:Date.now(),updatedAt:Date.now()};
    await updateDoc(ref,checked);
    return {id:snap.id,...data,...checked};
  },
