...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],notifications:[],
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.20 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.20 Firestore 규칙을 게시해주세요.';
...
async function prepareSchoolMap(item){
  ...
  clean.points=points;
  clean.layoutVersion=Number(clean.layoutVersion||2);
...
window.baemoonFirebase={
  ...
  async saveTimetable(payload){
    ...
    await setDoc(doc(db,'timetables',id),data,{merge:false});
    return data;
  },
