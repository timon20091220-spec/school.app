...
const state={screen:'home',loginRole:'student',currentFestivalId:null,managerFestivalId:null,editingFestivalId:null,editingBoothId:null,editingMenuId:null,editingEventId:null,editingNoticeId:null,editingTodayId:null,editingMealId:null,lastDayKey:null,pendingImage:{notice:'',booth:'',menu:'',community:''},reservationBoothId:null,communityCategory:'전체',communitySearch:'',currentPostId:null,pendingStudentSession:null,deferredPrompt:null};
...
function festivals(){return window.firebaseCache?.festivals??read(KEYS.festivals,[])}
function saveFestivals(v){
  write(KEYS.festivals,v);
  if(window.firebaseCache)window.firebaseCache.festivals=v;
  if(isAdmin()&&window.baemoonFirebase?.saveFestivals)window.baemoonFirebase.saveFestivals(v).catch(err=>toast(err.message||'축제 서버 저장에 실패했습니다.'));
...
function meals(){return window.firebaseCache?.meals??read(KEYS.meals,[])}
function saveMeals(v){
  write(KEYS.meals,v);
  if(window.firebaseCache)window.firebaseCache.meals=v;
  if(isAdmin()&&window.baemoonFirebase?.saveMeals)window.baemoonFirebase.saveMeals(v).catch(err=>toast(err.message||'급식 서버 저장에 실패했습니다.'));
}
...
$('#deleteMealButton').addEventListener('click',()=>{
  ...
  saveMeals(meals().filter(item=>item.id!==state.editingMealId));
  closeOverlay('mealEditor');
  renderMealCard();
  renderAdminMeals();
  toast('급식 정보를 삭제했습니다.');
