...
function installGuideContent(){
  ...
  const steps=ios
    ?['Safari 하단의 공유 버튼을 누릅니다.','메뉴에서 ‘홈 화면에 추가’를 선택합니다.','오른쪽 위 ‘추가’를 누릅니다.']
    :android
      ?['Chrome 오른쪽 위 점 3개 메뉴를 누릅니다.','‘앱 설치’ 또는 ‘홈 화면에 추가’를 선택합니다.','설치 확인 버튼을 누릅니다.']
      :['주소창 오른쪽의 앱 설치 아이콘을 찾습니다.','아이콘이 없으면 브라우저 메뉴에서 ‘앱 설치’를 선택합니다.','설치 후 바탕화면 또는 시작 메뉴에서 실행합니다.'];
  $('#installGuideText').textContent=state.deferredPrompt?'이 기기에서는 바로 앱 설치를 시작할 수 있습니다.':'파일을 다운로드하는 버튼이 아니라, 현재 웹사이트를 앱처럼 설치하는 기능입니다.';
  $('#installGuideSteps').innerHTML=steps.map((step,i)=>`<div class="install-guide-step"><b>${i+1}</b><span>${step}</span></div>`).join('');
  $('#runInstallPrompt').hidden=!state.deferredPrompt;
}
window.addEventListener('beforeinstallprompt',event=>{event.preventDefault();state.deferredPrompt=event});
window.addEventListener('appinstalled',()=>{$('#installButton').hidden=true;closeOverlay('installGuideModal');toast('배문고 앱이 설치되었습니다.')});
$('#installButton').addEventListener('click',()=>{installGuideContent();openOverlay('installGuideModal')});
$('#runInstallPrompt').addEventListener('click',async()=>{
  if(!state.deferredPrompt)return;
  state.deferredPrompt.prompt();
  await state.deferredPrompt.userChoice;
  state.deferredPrompt=null;
  closeOverlay('installGuideModal');
});
...
function renderGuestSettings(){const access=guestAccess();$$('[data-guest-setting]').forEach(i=>{i.checked=!!access[i.dataset.guestSetting];i.onchange=async()=>{const next={...guestAccess(),[i.dataset.guestSetting]:i.checked};try{await window.baemoonFirebase.saveGuestAccess(next);if(window.firebaseCache)window.firebaseCache.guestAccess=next;write(KEYS.guestAccess,next);markAdminSaved('게스트 공개 범위가 Firebase에 저장되었습니다.');applyRoleVisibility();renderHome();toast('게스트 공개 범위를 저장했습니다.')}catch(error){i.checked=!i.checked;toast(firebaseFriendlyMessage(error))}}})}
...
function notificationMatchesAudience(item){
  const s=session();if(!s)return false;if(item.ownerUid)return item.ownerUid===s.uid;
  if(item.audience==='전체'||!item.audience)return true;
  if(s.role==='admin')return true;
  return s.role==='student'&&item.audience===`${s.grade}학년`;
}
