...
function accounts(){return read(KEYS.accounts,[])}function saveAccounts(v){write(KEYS.accounts,v)}
...
function communityPosts(){return read(KEYS.community,[])}function saveCommunityPosts(v){write(KEYS.community,v)}function reports(){return read(KEYS.reports,[])}function saveReports(v){write(KEYS.reports,v)}
...
seed();populateStudentSelectors();migrateCommunityToNamed();state.lastDayKey=localDateKey();const s=session();if(s){enterApp(s.role==='admin'?'admin':'home')}else showAuthGate();
setInterval(()=>{const key=localDateKey();if(key!==state.lastDayKey){state.lastDayKey=key;renderHome();if(isAdmin())renderAdmin()}else{renderTodayCard();if(isAdmin()&&state.screen==='admin')renderAdminToday()}},30000);
