...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.15 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.15 Firestore 규칙을 게시해주세요.';
...
window.baemoonFirebase={
  ...
  async createReservation(payload){
    ...
    const reservationId=`${auth.currentUser.uid}_${payload.boothId}_${encodeURIComponent(payload.time||'즉시 예약')}`;
    const ref=doc(db,'reservations',reservationId);
    try{
      const snapshot=await getDoc(ref);
      if(snapshot.exists())throw new Error('DUPLICATE_RESERVATION');
