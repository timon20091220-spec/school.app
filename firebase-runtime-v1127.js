...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,homeBanners:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],queueEntries:[],adminReservationStats:null,reservationServerStatus:'checking',notifications:[],
  personalNotifications:[],notificationStates:{},notificationPreference:{enabled:true},media:{},ready:false
};

let authFlowInProgress=false;
let listenerStops=[];
let suggestionMessageStop=null;
let suggestionParts={public:[],own:[]};
let pendingPasswordProfile=null;
let reservationSlotSyncStarted=false;
const reservationRequestCache=new Map();
const slotStatusCache=new Map();
let adminDashboardLoadedAt=0;
let personalOverviewLoadedAt=0;
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.27 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return 'Firebase 권한 확인에 실패했습니다. 로그인 상태를 확인한 뒤 다시 시도해주세요.';
...
function guestDeviceId(){
  const key='bm_guest_device_id_v1';
  let value=localStorage.getItem(key);
  if(!value){
    value=`guest-device-${crypto.randomUUID()}`;
    localStorage.setItem(key,value);
  }
  return value;
}
function guestIdentityCode(deviceId,school,name){
  const source=`${deviceId}|${String(school||'').trim().toLowerCase()}|${String(name||'').trim().toLowerCase()}`;
  let hash=2166136261;
...
function reservationCheckInCode(){
...
const RESERVATION_API='https://baemoon-reservation-server.timon20091220.workers.dev';
function serverValue(row,...keys){
  for(const key of keys){if(row&&row[key]!==undefined&&row[key]!==null)return row[key]}
  return undefined;
}
function normalizeServerReservation(row={}){
  return {
    id:String(serverValue(row,'id')||''),
    slotId:String(serverValue(row,'slot_key','slotKey')||''),
    festivalId:String(serverValue(row,'festival_id','festivalId')||''),
    festivalName:String(serverValue(row,'festival_name','festivalName')||''),
    boothId:String(serverValue(row,'booth_id','boothId')||''),
    boothName:String(serverValue(row,'booth_name','boothName')||''),
    time:String(serverValue(row,'time')||'즉시 예약'),
    userUid:String(serverValue(row,'user_uid','userUid')||''),
    participantRole:String(serverValue(row,'participant_role','participantRole')||''),
    user:String(serverValue(row,'display_name','displayName','user')||'예약자'),
    name:String(serverValue(row,'display_name','displayName','name')||'예약자'),
    guestCode:String(serverValue(row,'guest_code','guestCode')||''),
    groupSize:Number(serverValue(row,'group_size','groupSize')||1),
    status:String(serverValue(row,'status')||'confirmed'),
    checkInCode:String(serverValue(row,'check_in_code','checkInCode')||''),
    checkedIn:Boolean(Number(serverValue(row,'checked_in','checkedIn')||0)),
    checkedInAt:Number(serverValue(row,'checked_in_at','checkedInAt')||0)||null,
    createdAt:Number(serverValue(row,'created_at','createdAt')||Date.now()),
    updatedAt:Number(serverValue(row,'updated_at','updatedAt')||Date.now())
  };
}
function normalizeServerQueue(row={}){
  return {
    id:String(serverValue(row,'id')||''),
    slotId:String(serverValue(row,'slot_key','slotKey')||''),
    festivalId:String(serverValue(row,'festival_id','festivalId')||''),
    ...
    userUid:String(serverValue(row,'user_uid','userUid')||''),
    participantRole:String(serverValue(row,'participant_role','participantRole')||''),
    displayName:String(serverValue(row,'display_name','displayName')||'대기자'),
    guestCode:String(serverValue(row,'guest_code','guestCode')||''),
    queueNumber:Number(serverValue(row,'queue_number','queueNumber')||0),
    status:String(serverValue(row,'status')||'waiting'),
    createdAt:Number(serverValue(row,'created_at','createdAt')||Date.now()),
    calledAt:Number(serverValue(row,'called_at','calledAt')||0)||null,
    checkedInAt:Number(serverValue(row,'checked_in_at','checkedInAt')||0)||null
  };
}
async function reservationServerRequest(path,{
  method='GET',body,forceToken=false,retry=true,cacheTtl=0,force=false,attempt=0
}={}){
  requireUser();
  const cacheKey=`${auth.currentUser.uid}|${method}|${path}`;
  const cached=reservationRequestCache.get(cacheKey);
  if(method==='GET'&&!force&&cacheTtl>0&&cached&&Date.now()-cached.savedAt<cacheTtl){
    return cached.payload;
  }

  const token=await auth.currentUser.getIdToken(forceToken);
  const headers={Authorization:`Bearer ${token}`,Accept:'application/json'};
  if(body!==undefined)headers['Content-Type']='application/json';

  let response;
  try{
    response=await fetch(`${RESERVATION_API}${path}`,{
      method,headers,body:body===undefined?undefined:JSON.stringify(body),cache:'no-store'
    });
  }catch(error){
    if(attempt<2){
      await new Promise(resolve=>setTimeout(resolve,300*(attempt+1)));
      return reservationServerRequest(path,{
        method,body,forceToken,retry,cacheTtl,force,attempt:attempt+1
      });
    }
    const wrapped=new Error('예약 서버에 연결할 수 없습니다. 인터넷 연결을 확인해주세요.');
    wrapped.code='reservation-server-network';
    throw wrapped;
  }
  ...
  if(response.status===401&&retry){
    return reservationServerRequest(path,{
      method,body,forceToken:true,retry:false,cacheTtl,force:true,attempt
    });
  }

  if([429,502,503,504].includes(response.status)&&attempt<2){
    const retryAfter=Math.min(1500,Number(response.headers.get('retry-after')||0)*1000||350*(attempt+1));
    await new Promise(resolve=>setTimeout(resolve,retryAfter));
    return reservationServerRequest(path,{
      method,body,forceToken,retry,cacheTtl,force:true,attempt:attempt+1
    });
  }

  if(!response.ok||payload?.ok===false){
    const wrapped=new Error(payload?.error?.message||`예약 서버 오류 (${response.status})`);
    wrapped.code=payload?.error?.code||`api-${response.status}`;
    wrapped.details=payload?.error?.details;
    throw wrapped;
  }

  if(method==='GET'&&cacheTtl>0){
    reservationRequestCache.set(cacheKey,{savedAt:Date.now(),payload});
  }else if(method!=='GET'){
    reservationRequestCache.clear();
    slotStatusCache.clear();
    adminDashboardLoadedAt=0;
    personalOverviewLoadedAt=0;
  }
  return payload;
}
function reservationSlotInput(festival,booth,time){
  return {
    festivalId:String(festival.id),festivalName:String(festival.name||'행사'),
    boothId:String(booth.id),boothName:String(booth.name||'부스'),
    time:String(time||'즉시 예약'),capacity:Math.max(1,Number(booth.capacity||1)),
    minPeople:Math.max(1,Number(booth.minPeople||1)),
    maxPeople:Math.max(1,Number(booth.maxPeople||booth.capacity||1)),
    openStart:String(booth.openStart||''),openEnd:String(booth.openEnd||''),
    bookingClosed:Boolean(booth.bookingClosed),waitlistEnabled:Boolean(booth.waitlistEnabled),
    congestion:String(booth.congestion||'auto'),active:true
  };
}
function festivalReservationSlots(items){
  return (items||[]).flatMap(festival=>(festival.booths||[]).flatMap(booth=>{
    const raw=Array.isArray(booth.times)&&booth.times.length?booth.times:['즉시 예약'];
    const times=raw.map(item=>typeof item==='string'?item:(item?.value||item?.time||'')).filter(Boolean);
    return (times.length?times:['즉시 예약']).map(time=>reservationSlotInput(festival,booth,time));
  }));
}
async function syncReservationSlots(items,{quiet=false}={}){
  requireAdmin();
  const slots=festivalReservationSlots(items);
  if(!slots.length){
    window.firebaseCache.reservationServerStatus='connected';
    if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
    return {ok:true,synced:0};
  }
  window.firebaseCache.reservationServerStatus='syncing';
  if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
  try{
    const result=await reservationServerRequest('/api/admin/sync-slots',{
      method:'POST',body:{slots,replaceFestival:true}
    });
    window.firebaseCache.reservationServerStatus='connected';
    if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
    if(!quiet)ui().toast(`예약 서버에 ${Number(result.synced||0)}개 회차를 동기화했습니다.`);
    return result;
  }catch(error){
    window.firebaseCache.reservationServerStatus='error';
    if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
    throw error;
  }
}
async function refreshReservationServerData(role=ui().session()?.role,{force=false}={}){
  if(!auth.currentUser)return {reservations:[],queue:[]};
  try{
    if(role==='admin'){
      if(!force&&adminDashboardLoadedAt&&Date.now()-adminDashboardLoadedAt<30000){
        return {
          reservations:window.firebaseCache.reservations||[],
          stats:window.firebaseCache.adminReservationStats||{}
        };
      }
      const payload=await reservationServerRequest('/api/admin/dashboard',{
        cacheTtl:30000,force
      });
      window.firebaseCache.reservations=(payload.reservations||[]).map(normalizeServerReservation);
      ...
      window.firebaseCache.reservationServerStatus='connected';
      ui().renderAdminReservations();
      if(typeof ui().renderAdminStats==='function')ui().renderAdminStats();
      if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
      return {
        reservations:window.firebaseCache.reservations,
        stats:window.firebaseCache.adminReservationStats
    ...
    if(['student','guest'].includes(role)){
      if(!force&&personalOverviewLoadedAt&&Date.now()-personalOverviewLoadedAt<20000){
        return {
          reservations:window.firebaseCache.reservations||[],
          queue:window.firebaseCache.queueEntries||[]
        };
      }
      const payload=await reservationServerRequest('/api/me/overview',{
        cacheTtl:20000,force
      });
      window.firebaseCache.reservations=(payload.reservations||[]).map(normalizeServerReservation);
      window.firebaseCache.queueEntries=(payload.queue||[]).map(normalizeServerQueue);
      personalOverviewLoadedAt=Date.now();
      window.firebaseCache.reservationServerStatus='connected';
      ui().renderMy();ui().renderHomeReservations();
      if(typeof ui().handleQueueEntries==='function')ui().handleQueueEntries(window.firebaseCache.queueEntries);
      return {
        reservations:window.firebaseCache.reservations,
        queue:window.firebaseCache.queueEntries
      };
    ...
    window.firebaseCache.reservationServerStatus='error';
    if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
    console.warn('Reservation server refresh failed:',error);
    throw error;
  }
  return {reservations:[],queue:[]};
}
async function refreshServerSlotStatus(festivalId,boothId,time='즉시 예약',{force=false}={}){
  const cacheKey=`${festivalId}|${boothId}|${time||'즉시 예약'}`;
  const cached=slotStatusCache.get(cacheKey);
  if(!force&&cached&&Date.now()-cached.savedAt<15000)return cached.record;
  const params=new URLSearchParams({festivalId:String(festivalId),boothId:String(boothId),time:String(time||'즉시 예약')});
  const payload=await reservationServerRequest(`/api/slot/status?${params}`,{cacheTtl:15000,force});
  const slot=payload.slot||{};
  const id=`${festivalId}_${boothId}_${encodeURIComponent(time||'즉시 예약')}`;
  const record={
    id,festivalId:String(festivalId),boothId:String(boothId),time:String(time||'즉시 예약'),
    capacity:Number(slot.capacity||1),minPeople:Number(slot.minPeople||1),
    maxPeople:Number(slot.maxPeople||slot.capacity||1),
    openStart:String(slot.openStart||''),openEnd:String(slot.openEnd||''),
    reservedPeople:Number(payload.reservedPeople||0),remaining:Number(payload.remaining||0),
    waitingCount:Number(payload.waitingCount||0),bookingClosed:Boolean(slot.bookingClosed),
    waitlistEnabled:Boolean(slot.waitlistEnabled),congestion:String(slot.congestion||'auto'),updatedAt:Date.now()
  };
  const list=window.firebaseCache.reservationSlots||(window.firebaseCache.reservationSlots=[]);
  const index=list.findIndex(item=>item.id===id);
  if(index>=0)list[index]=record;else list.push(record);
  slotStatusCache.set(cacheKey,{savedAt:Date.now(),record});
  return record;
}

function stopListeners(){reservationSlotSyncStarted=false;adminDashboardLoadedAt=0;personalOverviewLoadedAt=0;reservationRequestCache.clear();slotStatusCache.clear();listenerStops.forEach(stop=>{try{stop()}catch{}});listenerStops=[];if(suggestionMessageStop){try{suggestionMessageStop()}catch{}suggestionMessageStop=null;}suggestionParts={public:[],own:[]};Object.assign(window.firebaseCache,{communityPosts:[],users:[],reports:[],festivals:null,meals:null,notices:null,dailySchedules:null,guestAccess:null,homeBanners:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],queueEntries:[],adminReservationStats:null,reservationServerStatus:'checking',notifications:[],personalNotifications:[],notificationStates:{},notificationPreference:{enabled:true},media:{},ready:false})}
...
async function prepareSchoolMap(item){
  const clean=JSON.parse(JSON.stringify(item||{}));
  clean.backgroundImage=await persistMedia(clean.backgroundImage||'','school-map-background');
  const points=clean.points&&typeof clean.points==='object'&&!Array.isArray(clean.points)?clean.points:{};
  for(const [pointId,point] of Object.entries(points)){
    points[pointId]={
      ...point,
      photo:await persistMedia(point.photo||'','school-map-point'),
      backgroundImage:await persistMedia(point.backgroundImage||'','school-map-room')
    };
  }
  clean.points=points;
  clean.layoutVersion=Number(clean.layoutVersion||3);
...
async function migrateAdminLocalData(){
  let total=0;
  total+=await migrateCollectionIfEmpty("festivals","bm_festivals_v5",prepareFestival);
  total+=await migrateCollectionIfEmpty("meals","bm_meals_v10");
  total+=await migrateCollectionIfEmpty("notices","bm_notices_v5",prepareNotice);
  total+=await migrateCollectionIfEmpty("dailySchedules","bm_today_v5");

  const guestRef=doc(db,"settings","guestAccess");
  ...
  if(!guest.exists()){
    ...
    try{
      ...
      legacy={
        festivals:stored?.festivals!==false,
        meals:stored?.meals===true
      };
    }catch{}
  ...
  // v11.27:
  // 예약·대기번호는 Cloudflare Worker + D1 전용입니다.
  // 로그인 중 옛 Firestore reservations 컬렉션을 읽거나 이전하지 않습니다.
  return total;
}

async function migrateStudentLocalData(profile){
  let migrated=0;

  let localPosts=[];
  try{
    localPosts=JSON.parse(localStorage.getItem("bm_community_v7")||"[]");
  }catch{}

  if(Array.isArray(localPosts)){
    const remote=await getDocs(collection(db,"communityPosts"));
    const ids=new Set(remote.docs.map(item=>item.id));
    const operations=[];

    for(const post of localPosts.filter(item=>item.authorKey===profile.studentKey)){
      if(ids.has(String(post.id)))continue;
      const image=await persistMedia(post.image||"","community");
      const clean={
        ...
        authorKey:profile.studentKey,
        authorName:profile.name,
        authorGrade:Number(profile.grade),
        authorClass:Number(profile.classNo),
        authorNumber:Number(profile.number),
        anonymous:false,
      ...
      operations.push(batch=>
        batch.set(doc(db,"communityPosts",String(post.id)),clean)
      );
      migrated++;
    }
  ...
  // 예약 데이터는 Cloudflare Worker + D1에서만 관리하므로
  // 옛 로컬 예약을 Firestore reservations로 이전하지 않습니다.

  let localNotifications=[];
  try{
    localNotifications=JSON.parse(
      localStorage.getItem("bm_notifications_v5")||"[]"
    );
  }catch{}
  ...
  if(Array.isArray(localNotifications)){
    for(const notification of localNotifications){
      const ref=doc(
        ...
        "items",
        String(notification.id||crypto.randomUUID())
      );
...
function listenShared(role){
  ...
  listenerStops.push(onSnapshot(
    doc(db,'settings','homeBanners'),
    snapshot=>{
      const items=snapshot.exists()&&Array.isArray(snapshot.data()?.items)
        ?snapshot.data().items
        :[];
      window.firebaseCache.homeBanners=items;
      ui().renderHome();
      if(role==='admin'&&typeof ui().renderAdminHomeBanners==='function'){
        ui().renderAdminHomeBanners();
      }
    },
    error=>ui().toast(displayFirebaseError(error))
  ));
...
function listenNotifications(uid,role){
  const broadcastSource=role==='guest'
    ?query(collection(db,'broadcastNotifications'),where('audience','==','게스트'))
  ...
  listenerStops.push(onSnapshot(
    ...
    snapshot=>{
      window.firebaseCache.notificationPreference=snapshot.exists()?snapshot.data():{enabled:true,classChatEnabled:true};
      ui().renderNotifications();
    },
    error=>ui().toast(displayFirebaseError(error))
  ));
...
function listenQueueEntries(role,uid){
  listenerStops.push(onSnapshot(
    collection(db,'queueEntries'),
    snapshot=>{
      const items=snapshot.docs.map(plain);
      window.firebaseCache.queueEntries=items;
      if(typeof ui().handleQueueEntries==='function')ui().handleQueueEntries(items);
    },
    error=>ui().toast(displayFirebaseError(error))
  ));
}
...
function listenTimetables(role,profile){
  if(role==='admin'){
    listenerStops.push(onSnapshot(
      collection(db,'timetables'),
      snapshot=>{
        window.firebaseCache.timetables=snapshot.docs.map(plain);
        ui().renderAdminTimetable();
      },
      error=>ui().toast(displayFirebaseError(error))
    ));
    return;
  ...
  if(role==='student'&&profile){
    const timetableId=`g${profile.grade}-c${profile.classNo}`;
    listenerStops.push(onSnapshot(
      doc(db,'timetables',timetableId),
      snapshot=>{
        window.firebaseCache.timetables=snapshot.exists()?[plain(snapshot)]:[];
        ui().renderTimetable();
      },
      error=>ui().toast(displayFirebaseError(error))
    ));
  ...
  window.firebaseCache.timetables=[];
}
function listenClassChat(role,profile){
  if(role!=='student'||!profile){
    window.firebaseCache.classChatMessages=[];
    return;
  }
  const classKey=`g${profile.grade}-c${profile.classNo}`;
  ui().state.classChatInitialized=false;
  ui().state.classChatUnread=0;
  listenerStops.push(onSnapshot(
    collection(db,'classChats',classKey,'messages'),
    snapshot=>{
      const items=snapshot.docs.map(plain);
      if(typeof ui().handleClassChatMessages==='function')ui().handleClassChatMessages(items);
      else{
        window.firebaseCache.classChatMessages=items;
        ui().renderClassChat();
      }
    },
    error=>ui().toast(displayFirebaseError(error))
...
function listenSchoolMaps(role){
  if(!['student','admin'].includes(role)){
    window.firebaseCache.schoolMaps=[];
    return;
  }
  listenerStops.push(onSnapshot(
    collection(db,'schoolMaps'),
    snapshot=>{
      window.firebaseCache.schoolMaps=snapshot.docs.map(plain);
      ui().state.adminMapDrafts={};
      if(ui().state.screen==='guide'&&typeof ui().renderGuide==='function')ui().renderGuide();
      if(role==='admin'&&typeof ui().renderAdminMap==='function')ui().renderAdminMap();
    },
    error=>ui().toast(displayFirebaseError(error))
  ));
}
function mergeSuggestionParts(){
  const merged=new Map();
  [...suggestionParts.public,...suggestionParts.own].forEach(item=>merged.set(item.id,item));
  window.firebaseCache.suggestions=[...merged.values()];
  if(typeof ui().renderSuggestions==='function')ui().renderSuggestions();
  if(typeof ui().renderAdminSuggestions==='function'&&ui().session()?.role==='admin')ui().renderAdminSuggestions();
}
function listenSuggestions(role,uid){
  if(role==='admin'){
    listenerStops.push(onSnapshot(
      collection(db,'suggestions'),
      snapshot=>{
        suggestionParts.public=snapshot.docs.map(plain);
        suggestionParts.own=[];
        mergeSuggestionParts();
      },
      error=>ui().toast(displayFirebaseError(error))
    ));
    return;
  ...
  if(role==='student'){
    listenerStops.push(onSnapshot(
      query(collection(db,'suggestions'),where('visibility','==','public')),
      snapshot=>{suggestionParts.public=snapshot.docs.map(plain);mergeSuggestionParts()},
      error=>ui().toast(displayFirebaseError(error))
    ));
    listenerStops.push(onSnapshot(
      query(collection(db,'suggestions'),where('ownerUid','==',uid)),
      snapshot=>{suggestionParts.own=snapshot.docs.map(plain);mergeSuggestionParts()},
      error=>ui().toast(displayFirebaseError(error))
    ));
    return;
  }
  suggestionParts={public:[],own:[]};
  window.firebaseCache.suggestions=[];
}
function startDataListeners(role,profile=null){
  stopListeners();
  listenShared(role);
  listenNotifications(auth.currentUser.uid,role);
  window.firebaseCache.reservations=[];
  window.firebaseCache.reservationSlots=[];
  window.firebaseCache.queueEntries=[];
  refreshReservationServerData(role).catch(error=>console.warn('Initial reservation server load failed:',error));
  listenTimetables(role,profile);
  listenClassChat(role,profile);
  listenSchoolMaps(role);
  listenSuggestions(role,auth.currentUser.uid);
  ...
  if(role==='admin'){
    listenerStops.push(onSnapshot(
      ...
      snapshot=>{
        window.firebaseCache.users=snapshot.docs.map(item=>({
          uid:item.id,
          ...item.data(),
          guestCode:item.data().guestCode||guestIdentityCode(
            item.data().deviceId||item.id,item.data().school,item.data().name
          ),
    ...
    listenerStops.push(onSnapshot(
      collection(db,'guestProfiles'),
      snapshot=>{
        window.firebaseCache.guestProfiles=snapshot.docs.map(item=>({
          uid:item.id,
          ...item.data(),
          lastLoginAt:normalizeDate(item.data().lastLoginAt)
        }));
    ...
    window.firebaseCache.guestProfiles=[];
  }else{
    window.firebaseCache.guestProfiles=[];
  }
...
async function finishStudentLogin(user,expected){
  ...
  if(!snap.exists()){
    profile={
      ...
      classKey:`g${expected.grade}-c${expected.classNo}`,
    ...
    await updateDoc(ref,{lastLoginAt:Date.now(),classKey:`g${expected.grade}-c${expected.classNo}`});
...
async function submitGuestLogin(){
  ...
  try{
    ...
    const profile={
      ...
      name,
      deviceId,
      guestCode:guestIdentityCode(deviceId,school,name),
      createdAt:now,
      lastLoginAt:now
    };
...
onAuthStateChanged(auth,async user=>{
  ...
  try{
    ...
    if(user.isAnonymous){
      ...
      const guestCode=guestProfile.guestCode||guestIdentityCode(
        deviceId,guestProfile.school,guestProfile.name
      );
      await updateDoc(doc(db,'guestProfiles',user.uid),{
        lastLoginAt:now,deviceId,guestCode
      });
      ui().setSession({...guestProfile,deviceId,guestCode,lastLoginAt:now});
...
window.baemoonFirebase={
  ...
  async saveTimetable(payload){
    ...
    const id=String(payload.id||`g${grade}-c${classNo}`);
    const keys=['mon','tue','wed','thu','fri'];
    const raw=payload.days||{};
    const days=Object.fromEntries(keys.map((key,index)=>[
      key,Array.from({length:7},(_,period)=>{
        const value=Array.isArray(raw)?raw?.[index]?.[period]:raw?.[key]?.[period];
        return String(value||'').slice(0,30);
      })
    ]));
    const data={id,grade,classNo,days,updatedAt:Date.now(),updatedBy:auth.currentUser.uid};
    await setDoc(doc(db,'timetables',id),data,{merge:true});
    return data;
  },
  async sendClassChatMessage(payload){
    requireUser();
    const current=ui().session();
    if(current?.role!=='student')throw new Error('학생만 반별 채팅을 이용할 수 있습니다.');
    const classKey=`g${current.grade}-c${current.classNo}`;
    if(payload.classKey&&payload.classKey!==classKey)throw new Error('본인 학급 채팅방만 이용할 수 있습니다.');
    const body=String(payload.body||'').trim();
    const image=String(payload.image||'');
    if(image&&(!image.startsWith('data:image/')||image.length>800000)){
      throw new Error('채팅 사진 형식 또는 용량을 확인해주세요.');
    }
    if(!body&&!image)throw new Error('메시지 또는 사진을 입력해주세요.');
    if(body.length>300)throw new Error('메시지는 300자 이하로 작성해주세요.');
    const ref=doc(collection(db,'classChats',classKey,'messages'));
    const data={
      id:ref.id,classKey,
      grade:Number(current.grade),classNo:Number(current.classNo),
      authorUid:auth.currentUser.uid,authorKey:current.studentKey,
      authorName:current.name,
      authorLabel:`${current.grade}학년 ${current.classNo}반 ${current.number}번 ${current.name}`,
      body,image,createdAt:Date.now()
    };
    await setDoc(ref,data);
    return data;
  },
  async setClassChatNotificationPreference(enabled){
    requireUser();
    await setDoc(
      doc(db,'notificationPreferences',auth.currentUser.uid),
      {classChatEnabled:Boolean(enabled),updatedAt:Date.now()},
      {merge:true}
    );
    return Boolean(enabled);
  },
  async saveSchoolMap(payload){
    requireAdmin();
    const prepared=await prepareSchoolMap(payload);
    const id=String(prepared.id||`floor-${Number(prepared.floor||1)}`);
    prepared.id=id;
    await setDoc(doc(db,'schoolMaps',id),prepared,{merge:false});
    return prepared;
  },
  async createSuggestion(payload){
    requireUser();
    const current=ui().session();
    if(current?.role!=='student')throw new Error('학생만 건의를 작성할 수 있습니다.');
    ...
    const data={
      id:ref.id,visibility,title,preview:body.slice(0,180),
      ownerUid:auth.currentUser.uid,status:'접수',
      createdAt:Date.now(),updatedAt:Date.now()
    };
    const messageRef=doc(collection(db,'suggestions',ref.id,'messages'));
    const batch=writeBatch(db);
    batch.set(ref,data);
    batch.set(messageRef,{
      id:messageRef.id,suggestionId:ref.id,
      senderUid:auth.currentUser.uid,senderRole:'student',
      body,createdAt:Date.now()
    });
    await batch.commit();
    return data;
  },
  watchSuggestionMessages(suggestionId){
    requireUser();
    if(suggestionMessageStop){try{suggestionMessageStop()}catch{}}
    suggestionMessageStop=onSnapshot(
      collection(db,'suggestions',String(suggestionId),'messages'),
      snapshot=>{
        const items=snapshot.docs.map(plain);
        window.firebaseCache.suggestionMessages=items;
        if(typeof ui().handleSuggestionMessages==='function')ui().handleSuggestionMessages(items);
      },
      error=>ui().toast(displayFirebaseError(error))
    );
    return true;
  },
  async sendSuggestionMessage(suggestionId,bodyValue){
    requireUser();
    const body=String(bodyValue||'').trim();
    if(!body)throw new Error('메시지를 입력해주세요.');
    if(body.length>500)throw new Error('메시지는 500자 이하로 작성해주세요.');
    const suggestionRef=doc(db,'suggestions',String(suggestionId));
    const suggestionSnap=await getDoc(suggestionRef);
    if(!suggestionSnap.exists())throw new Error('건의를 찾지 못했습니다.');
    const messageRef=doc(collection(db,'suggestions',String(suggestionId),'messages'));
    const senderRole=isAdminUser()?'admin':'student';
    const batch=writeBatch(db);
    batch.set(messageRef,{
      id:messageRef.id,suggestionId:String(suggestionId),
      senderUid:auth.currentUser.uid,senderRole,body,createdAt:Date.now()
    });
    batch.update(suggestionRef,{updatedAt:Date.now(),lastMessage:body.slice(0,120)});
    await batch.commit();
  ...
  async createReservation(payload){
    requireUser();
    const current=ui().session();
    if(!['student','guest'].includes(current?.role))throw new Error('학생 또는 게스트만 예약할 수 있습니다.');
    const result=await reservationServerRequest('/api/reserve',{
      method:'POST',body:{
        festivalId:String(payload.festivalId),boothId:String(payload.boothId),
        time:String(payload.time||'즉시 예약'),groupSize:Number(payload.groupSize||1),
        displayName:String(payload.user||current.name||'예약자'),guestCode:String(current.guestCode||'0000'),
        autoQueue:true,requestId:crypto.randomUUID()
      }
    });
    if(result.outcome==='confirmed'&&result.reservation){
      const reservation=normalizeServerReservation(result.reservation);
      const list=window.firebaseCache.reservations||(window.firebaseCache.reservations=[]);
      const index=list.findIndex(item=>item.id===reservation.id||(
        item.boothId===reservation.boothId&&item.time===reservation.time&&item.userUid===auth.currentUser.uid
      ));
      if(index>=0)list[index]=reservation;else list.push(reservation);
      await refreshServerSlotStatus(payload.festivalId,payload.boothId,payload.time||'즉시 예약').catch(()=>{});
      return {...result,reservation};
    }
    if(result.outcome==='waitlisted'&&result.queue){
      const queue=normalizeServerQueue({
        ...result.queue,festivalId:payload.festivalId,festivalName:payload.festivalName,
        boothId:payload.boothId,boothName:payload.boothName,time:payload.time||'즉시 예약',
        userUid:auth.currentUser.uid,participantRole:current.role,displayName:payload.user,
        guestCode:current.guestCode||'0000'
      });
      const list=window.firebaseCache.queueEntries||(window.firebaseCache.queueEntries=[]);
      const index=list.findIndex(item=>item.id===queue.id);
      if(index>=0)list[index]=queue;else list.push(queue);
      return {...result,queue};
    }
    return result;
  },
  async cancelReservation(reservationId){
    requireUser();
    const reservation=(window.firebaseCache.reservations||[]).find(item=>item.id===reservationId);
    if(!reservation)throw new Error('예약 정보를 찾지 못했습니다. 예약 목록을 새로고침해주세요.');
    const result=await reservationServerRequest('/api/cancel',{
      method:'POST',body:{festivalId:reservation.festivalId,boothId:reservation.boothId,time:reservation.time||'즉시 예약'}
    });
    window.firebaseCache.reservations=(window.firebaseCache.reservations||[]).filter(item=>item.id!==reservationId);
    await refreshServerSlotStatus(reservation.festivalId,reservation.boothId,reservation.time||'즉시 예약').catch(()=>{});
    ui().renderMy();ui().renderHomeReservations();
    return result;
  },
  async sendReservationMessage(reservationId,payload){
    requireAdmin();
    const reservation=(window.firebaseCache.reservations||[]).find(item=>item.id===reservationId);
    if(!reservation)throw new Error('예약 정보를 찾지 못했습니다.');
    ...
    const data={
      id:ref.id,title:String(payload.title||'예약 안내'),body:String(payload.body||''),
      type:'reservation',audience:'개인',ownerUid:reservation.userUid,senderUid:auth.currentUser.uid,
      reservationId:reservation.id,boothName:reservation.boothName||'',createdAt:Date.now()
    };
    await setDoc(ref,data);return data;
  },
  async clearReservations(){
    throw new Error('Cloudflare 예약 서버의 전체 초기화 기능은 안전을 위해 비활성화되어 있습니다.');
  },
  async joinBoothQueue(payload){
    requireUser();
    const current=ui().session();
    const result=await reservationServerRequest('/api/queue/join',{
      method:'POST',body:{
        festivalId:String(payload.festivalId),boothId:String(payload.boothId),time:String(payload.time||'즉시 예약'),
        displayName:String(payload.displayName||current?.name||'대기자'),guestCode:String(current?.guestCode||'0000'),
    ...
    const queue=normalizeServerQueue({
      ...result.queue,festivalId:payload.festivalId,festivalName:payload.festivalName,
      boothId:payload.boothId,boothName:payload.boothName,time:payload.time||'즉시 예약',
      userUid:auth.currentUser.uid,participantRole:current?.role,displayName:payload.displayName||current?.name,
      guestCode:current?.guestCode||'0000'
    });
    const list=window.firebaseCache.queueEntries||(window.firebaseCache.queueEntries=[]);
    const index=list.findIndex(item=>item.id===queue.id);
    if(index>=0)list[index]=queue;else list.push(queue);
    if(typeof ui().handleQueueEntries==='function')ui().handleQueueEntries(list);
    return queue;
  },
  async cancelQueueEntry(entryId){
    requireUser();
    const entry=(window.firebaseCache.queueEntries||[]).find(item=>item.id===entryId);
    if(!entry)throw new Error('대기번호 정보를 찾지 못했습니다.');
    const result=await reservationServerRequest('/api/queue/cancel',{
      method:'POST',body:{festivalId:entry.festivalId,boothId:entry.boothId,time:entry.time||'즉시 예약'}
    });
    window.firebaseCache.queueEntries=(window.firebaseCache.queueEntries||[]).filter(item=>item.id!==entryId);
    if(typeof ui().handleQueueEntries==='function')ui().handleQueueEntries(window.firebaseCache.queueEntries);
    return result;
  },
  async callNextQueue(payload){
    requireAdmin();
    return reservationServerRequest('/api/admin/queue/call-next',{
      method:'POST',body:{festivalId:payload.festivalId,boothId:payload.boothId,time:payload.time||'즉시 예약'}
    });
  },
  async updateQueueStatus(entryId,status){
    requireAdmin();
    const entry=(window.firebaseCache.queueEntries||[]).find(item=>item.id===entryId);
    if(status==='checkedin'){
      const result=await reservationServerRequest('/api/admin/queue/check-in',{method:'POST',body:{queueId:entryId}});
      window.firebaseCache.queueEntries=(window.firebaseCache.queueEntries||[]).filter(item=>item.id!==entryId);
      return result;
    }
    if(status==='called'&&entry){return this.callNextQueue(entry)}
    throw new Error('지원하지 않는 대기번호 상태 변경입니다.');
  },
  async ensureReservationCheckInCode(reservationId){
    requireUser();
    const reservation=(window.firebaseCache.reservations||[]).find(item=>item.id===reservationId);
    if(!reservation)throw new Error('예약 정보를 찾지 못했습니다.');
    if(!reservation.checkInCode)throw new Error('체크인 코드가 비어 있습니다. 예약 목록을 새로고침해주세요.');
    return reservation;
  },
  async checkInReservationCode(rawCode){
    requireAdmin();
    const result=await reservationServerRequest('/api/admin/check-in',{method:'POST',body:{payload:String(rawCode||'').trim()}});
    const reservation=normalizeServerReservation(result.reservation||{});
    const list=window.firebaseCache.reservations||(window.firebaseCache.reservations=[]);
    const index=list.findIndex(item=>item.id===reservation.id);
    if(index>=0)list[index]=reservation;else list.push(reservation);
  ...
  async refreshReservationData(options={}){return refreshReservationServerData(ui().session()?.role,options)},
  async getSlotStatus(payload){return refreshServerSlotStatus(payload.festivalId,payload.boothId,payload.time||'즉시 예약',{force:Boolean(payload.force)})},
  async syncReservationSlots(items){return syncReservationSlots(items||window.firebaseCache.festivals||[])},
  async getAdminStats(options={}){requireAdmin();await refreshReservationServerData('admin',options);return window.firebaseCache.adminReservationStats},
  async getAdminReservations(filters={}){
    requireAdmin();
    const params=new URLSearchParams();
    for(const key of ['festivalId','boothId','time'])if(filters[key])params.set(key,String(filters[key]));
    const payload=await reservationServerRequest(`/api/admin/reservations${params.toString()?`?${params}`:''}`,{
      cacheTtl:15000,force:Boolean(filters.force)
    });
    return (payload.reservations||[]).map(normalizeServerReservation);
  },
  async getAdminSlotQueue(payload){
    requireAdmin();
    const params=new URLSearchParams({festivalId:String(payload.festivalId),boothId:String(payload.boothId),time:String(payload.time||'즉시 예약')});
    const result=await reservationServerRequest(`/api/admin/slot-queue?${params}`,{
      cacheTtl:10000,force:Boolean(payload.force)
    });
    return (result.queue||[]).map(normalizeServerQueue);
  },
  async getAdminBoothOverview(payload){
    requireAdmin();
    const params=new URLSearchParams({
      festivalId:String(payload.festivalId),
      boothId:String(payload.boothId),
      time:String(payload.time||'즉시 예약')
    });
    const result=await reservationServerRequest(`/api/admin/booth-overview?${params}`,{
      cacheTtl:10000,force:Boolean(payload.force)
    });
    return {
      reservations:(result.reservations||[]).map(normalizeServerReservation),
      queue:(result.queue||[]).map(normalizeServerQueue)
    };
  },
  async reservationServerHealth(){
    const response=await fetch(`${RESERVATION_API}/health`,{cache:'no-store'});
    if(!response.ok)throw new Error('예약 서버 상태를 확인할 수 없습니다.');
    return response.json();
  },
  async saveHomeBanners(items){
    requireAdmin();
    const prepared=[];
    for(const item of (items||[]).slice(0,3)){
      prepared.push({
        id:String(item.id||crypto.randomUUID()),
        kicker:String(item.kicker||'').slice(0,40),
        title:String(item.title||'').slice(0,80),
        subtitle:String(item.subtitle||'').slice(0,160),
        buttonLabel:String(item.buttonLabel||'').slice(0,30),
        link:String(item.link||'').slice(0,500),
        color:String(item.color||'#111827').slice(0,20),
        image:await persistMedia(item.image||'','home-banner'),
        visible:item.visible!==false,
        updatedAt:Number(item.updatedAt||Date.now())
      });
    }
    await setDoc(doc(db,'settings','homeBanners'),{items:prepared,updatedAt:Date.now()},{merge:false});
    window.firebaseCache.homeBanners=prepared;
    return prepared;
  },
  async recordPrivacyConsent(payload){
    requireUser();
    const data={
      uid:auth.currentUser.uid,
      role:ui().session()?.role||'unknown',
    ...
    await setDoc(doc(db,'consents',auth.currentUser.uid),data,{merge:true});
    return data;
  },
