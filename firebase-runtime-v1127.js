/* 배문고 Firebase Runtime v12.05 performance loader
...
const CACHE_KEY='baemoon-firebase-runtime-68b50607-v1205';
const OLD_CACHE_KEYS=['baemoon-firebase-runtime-68b50607-v1204','baemoon-firebase-runtime-68b50607-v1203','baemoon-firebase-runtime-68b50607-v1202','baemoon-firebase-runtime-68b50607-v1201','baemoon-firebase-runtime-68b50607-v1144','baemoon-firebase-runtime-68b50607-v1143','baemoon-firebase-runtime-68b50607-v1142','baemoon-firebase-runtime-68b50607-v1141'];
...
function optimizeRuntime(source){
  ...
  source=source.replace(
    ...
    `const auth=getAuth(firebaseApp);
const GUEST_SIGNED_OUT_KEY='bm_guest_signed_out_v1205';
...
function newGuestAuthEmail(){const id=(crypto.randomUUID?.()||Math.random().toString(36).slice(2)+Date.now().toString(36)).replace(/[^a-zA-Z0-9]/g,'').toLowerCase();return 'guest-'+id+'@'+GUEST_EMAIL_DOMAIN}
...
window.baemoonGuestAccountInfo=()=>readGuestAccountLock();`
  ...
  // v12.05: 학생 계정과 같은 Firebase Email/Password 기반 게스트 로그인.
  ...
  const passwordInput=$('#guestPasswordV1205')||$('#guestPasswordV1204');
  const newPasswordInput=$('#guestNewPasswordV1205')||$('#guestNewPasswordV1204');
  const confirmInput=$('#guestNewPasswordConfirmV1205')||$('#guestNewPasswordConfirmV1204');
  ...
  try{
    ...
    if(lock?.uid&&current&&String(lock.uid)!==String(current.uid))throw new Error('이 브라우저에는 다른 게스트 계정이 연결되어 있습니다. 관리자에게 문의해주세요.');
    ...
    const validateFirstPassword=()=>{
      if(entered!==COMMON_PASSWORD)throw new Error('첫 로그인 비밀번호는 baemoon2026입니다. 이미 개인 비밀번호를 설정했다면 그 비밀번호를 입력해주세요.');
      if(newPassword.length<8)throw new Error('개인 비밀번호는 8자 이상이어야 합니다.');
      if(newPassword===COMMON_PASSWORD)throw new Error('최초 비밀번호와 다른 개인 비밀번호를 설정해주세요.');
      if(newPassword!==confirmation)throw new Error('개인 비밀번호 확인이 일치하지 않습니다.');
      return newPassword;
    };
    let user=current,authEmail=String(lock?.authEmail||existing?.authEmail||'').toLowerCase();
    if(current?.isAnonymous){
      if(lock&&!existing)throw new Error('기존 게스트 정보를 Firebase에서 찾지 못했습니다. 관리자에게 문의해주세요.');
      let personalPassword='';
      if(existing?.passwordHash&&existing?.passwordSalt){
        const actual=await guestPasswordDigest(entered,existing.passwordSalt);
        if(!actual||actual!==String(existing.passwordHash))throw new Error('개인 비밀번호가 올바르지 않습니다.');
        personalPassword=entered;
      }else personalPassword=validateFirstPassword();
      authEmail=authEmail||newGuestAuthEmail();
      try{user=(await linkWithCredential(current,EmailAuthProvider.credential(authEmail,personalPassword))).user}
      catch(error){
        if(String(error?.code||'').includes('credential-already-in-use')||String(error?.code||'').includes('email-already-in-use'))throw new Error('이 브라우저의 게스트 계정 연결 정보가 이미 사용 중입니다. 관리자에게 문의해주세요.');
        throw error;
      }
      existing=existing||{};
    }else if(isGuestEmailUser(current)){
      user=current;authEmail=String(current.email||authEmail).toLowerCase();existing=existing||await getGuestProfile(current);
    }else if(authEmail){
      user=(await signInWithEmailAndPassword(auth,authEmail,entered)).user;
      existing=await getGuestProfile(user);
      if(!existing)throw new Error('게스트 계정 정보가 Firebase에서 삭제되었습니다. 관리자에게 문의해주세요.');
    }else{
      const personalPassword=validateFirstPassword();
      authEmail=newGuestAuthEmail();
      user=(await createUserWithEmailAndPassword(auth,authEmail,personalPassword)).user;
      existing={};
    }
    ...
    const profile={uid:user.uid,role:'guest',school,name,deviceId,guestCode:'',authEmail,authMode:'password',usesCustomPassword:true,passwordSalt:'',passwordHash:'',passwordChangedAt:existing?.passwordChangedAt||now,createdAt:existing?.createdAt||now,lastLoginAt:now};
    await setDoc(doc(db,'guestProfiles',user.uid),profile,{merge:true});
    writeGuestAccountLock({uid:user.uid,authEmail,school,name,createdAt:lock?.createdAt||profile.createdAt});
...
window.baemoonSetLoginPersistence=applyLoginPersistence;`)
  ...
  console.error('배문고 Firebase Runtime v12.05 시작 실패:',error);
