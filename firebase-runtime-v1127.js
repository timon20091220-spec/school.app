/* 배문고 Firebase Runtime v11.34 loader
...
const CACHE_KEY='baemoon-firebase-runtime-68b50607-v1134';
...
try{
  ...
  source=source.replace(pattern,`${SLOT_FUNCTION}\nfunction festivalReservationSlots`)
    .replace(/v11\.27/g,'v11.34');
  const blobUrl=URL.createObjectURL(new Blob([source],{type:'text/javascript'}));
  try{await import(blobUrl)}finally{URL.revokeObjectURL(blobUrl)}
}catch(error){
  console.error('배문고 Firebase Runtime v11.34 시작 실패:',error);
