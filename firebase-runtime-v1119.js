...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.19 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.19 Firestore 규칙을 게시해주세요.';
...
async function prepareSchoolMap(item){
  const clean=JSON.parse(JSON.stringify(item||{}));
  clean.image=await persistMedia(clean.image||'','school-map');
  ...
  for(const [pointId,point] of Object.entries(points)){
    points[pointId]={...point,photo:await persistMedia(point.photo||'','school-map-point')};
  }
  clean.points=points;
