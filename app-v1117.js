...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1117.js',{scope:'./',updateViaCache:'none'})
...
function isAndroidInAppBrowser(){
  ...
  return isAndroidDevice()&&(
    /\bwv\b/i.test(ua)
    ||/FBAN|FBAV|Instagram|KAKAOTALK|NAVER|Line|Daum|ChatGPT/i.test(ua)
  );
}
...
function waitForInstallPrompt(timeoutMs=15000){
...
function openChromeForInstall(){
  const target=`${location.host}${location.pathname}?install=1`;
  const fallback=encodeURIComponent(`${location.origin}${location.pathname}?install=1`);
...
function showInstallFallback(){
  ...
  const chromeButton=$('#openChromeInstallButton');

  if(chromeButton){
    chromeButton.hidden=!isAndroidDevice();
  }

  if(isAndroidInAppBrowser()){
    $('#installHelpText').textContent='현재 앱 안의 브라우저에서는 설치창을 열 수 없습니다. 아래 버튼으로 Chrome을 연 뒤 ‘앱으로 설치’를 다시 눌러주세요.';
  }else if(isIOS||isSafari){
    $('#installHelpText').textContent='Safari의 공유 버튼을 누른 뒤 ‘홈 화면에 추가’를 선택하면 일반 앱처럼 설치됩니다.';
  }else{
    $('#installHelpText').textContent='이 브라우저가 아직 설치창을 제공하지 않았습니다. 페이지를 새로고침하거나 브라우저 메뉴의 ‘앱 설치’를 선택해주세요.';
  }
...
async function triggerPwaInstall(){
  ...
  try{
    ...
    const promptEvent=await waitForInstallPrompt(15000);
    if(!promptEvent){
      showInstallFallback();
      return;
    }
...
$('#openChromeInstallButton').addEventListener('click',openChromeForInstall);
if(new URLSearchParams(location.search).get('install')==='1'){
  setTimeout(()=>toast('Chrome에서 앱으로 설치 버튼을 눌러주세요.'),700);
}
    ...
    ?entries.map(entry=>{
      ...
      return `<button class="admin-booth-reservation-card" data-admin-booth-reservations="${entry.key}">
        ...
        <div class="admin-booth-slot-preview">
          ${slots.map(slot=>`<i class="${slot.people>=slot.capacity?'slot-full':''}">${esc(slot.time)} ${slot.people}/${slot.capacity}</i>`).join('')}
        </div>
