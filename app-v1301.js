...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1301.js?v=1301',{scope:'./',updateViaCache:'none'})
...
function openBoothEditor(bid=null){
  ...
  ensureRebookingControlV1300();
  $('#boothAllowRebookingV1300').value='no';
  ...
  if(bid){
    ...
    ensureRebookingControlV1300();
    $('#boothAllowRebookingV1300').value=booth.allowRebooking?'yes':'no';
...
document.addEventListener('click',async event=>{
  ...
  const payload={
    ...
    allowRebooking:$('#boothAllowRebookingV1300')?.value==='yes',
...
function queueUserLabel(entry){
  ...
  if(guest)return `${guest.name||'게스트'}`;
...
(()=>{
  ...
  function installApiExtensions(){
    ...
    api.callNextQueue=async payload=>{
      ...
      /* v13.00: 호출 알림은 Cloudflare Web Push 한 경로에서만 전송합니다. */
      return result;
    };
...
(()=>{
  ...
  window.__bmUiWordsV1135=runWordsObserverV1141;
...
(()=>{
  ...
  window.__bmUiV1136=runFeatureObserverV1141;
...
(()=>{
  ...
  function renderMyParticipationsCompact(){
    ...
    list.dataset.v1143Signature=signature;
    list.innerHTML=items.length?items.map(item=>{const review=reviews.find(row=>String(row.participation_id||row.participationId||'')===String(item.id));return `<article class="my-participation-card-v1143"><h3>${escV(item.booth_name||item.boothName||'부스')}</h3><button type="button" data-write-review-v1136="${escV(item.id)}">${review?'후기 보기':'후기'}</button></article>`}).join(''):'<div class="festival-empty">아직 참여 완료된 부스가 없습니다.</div>';
  }
...
(()=>{
  ...
  function renderMyParticipationsV1144(){
    ...
    list.innerHTML=items.length?items.map(item=>{
      ...
      return `<article class="my-participation-card-v1143 my-participation-card-v1144"><span class="participation-festival-v1144">${escV(festival)}</span><h3>${escV(booth)}</h3><button type="button" data-write-review-v1136="${escV(item.id)}">${review?'후기 보기':'후기'}</button></article>`;
    }).join(''):'<div class="festival-empty">아직 참여 완료된 부스가 없습니다.</div>';
  }
  ...
  function identityForReview(item){
    ...
    if(item.participant_role==='guest'){
      return {id:'게스트',name:raw.replace(/\s*#\d{3,8}\s*$/,'')||'게스트'};
    }
  ...
  window.__bmUiV1144=runUiV1144;

  (async()=>{
    try{
      await waitReady();installApi();runUiV1144();
      setInterval(()=>{
        const role=window.baemoonApp?.session?.()?.role;
...
(()=>{
  ...
  function setVersionV1201(){
    ...
    document.querySelectorAll('[data-app-version],.app-version,.firebase-beta-status+small,.auth-welcome-card>small').forEach(element=>{
      ...
      if(/v11\.(?:27|33|34|35|36|42|43|44)/.test(text))element.textContent=text.replace(/v11\.(?:27|33|34|35|36|42|43|44)/g,'v12.05');
    });
  }
  ...
  window.__bmUiV1201=runUiV1201;
  (async()=>{
    for(let i=0;i<80&&!document.body;i++)await wait(50);
    runUiV1201();setInterval(()=>{if(!document.hidden)updateFestivalManagerLabelsV1201()},5000);
    console.info('Baemoon v12.05 features ready');
  })();
})();
...
(()=>{
  ...
  async function registerLatestDevice1202(){ return true; /* v13.01: v11.44 all-subscriptions mode */ }
  ...
  function accountLabel1202(){
    ...
    if(current.role==='guest')return `${current.name||current.displayName||'게스트'}`;
  ...
  window.addEventListener('change',event=>{if(event.target?.matches?.('#notificationToggle')){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();(window.__handleNotificationToggleV1205?window.__handleNotificationToggleV1205(event.target.checked):(window.__toggleNotificationsV1204?window.__toggleNotificationsV1204(event.target.checked):toggleNotifications1202(event.target.checked))) }},true);
  window.addEventListener('click',event=>{
  ...
  function ui1202(){
    ...
    document.querySelectorAll('[data-app-version],.app-version').forEach(element=>element.textContent=(element.textContent||'').replace(/v?(?:11\.\d+|12\.0[1-5])/g,'v13.01'));
    const logoutText=document.querySelector('#logoutButton b');if(logoutText)logoutText.textContent='로그아웃';
    if(window.baemoonApp?.state?.screen==='festival'){decorateRatings1202();renderAdminReviews1202(false)}
  }
  window.__bmUiV1202=ui1202;
  (async()=>{
    for(let i=0;i<180&&!window.__firebaseRuntimeReady;i++)await sleep1202(100);
    installApi1202();ui1202();
    const current=window.baemoonApp?.session?.();if(current&&'Notification'in window&&Notification.permission==='granted')saveCurrentPush1202().catch(()=>{});
    let lastSessionUid1202=String(current?.uid||'');setInterval(()=>{const uid=String(window.baemoonApp?.session?.()?.uid||'');if(uid&&uid!==lastSessionUid1202){lastSessionUid1202=uid;if('Notification'in window&&Notification.permission==='granted')saveCurrentPush1202().catch(()=>{})}else if(!uid)lastSessionUid1202=''},8000);
    ...
    console.info('Baemoon v12.05 features ready');
  })().catch(error=>console.error('v12.05 initialization failed',error));
})();


/* ===== v12.05 admin rosters · check-in refresh · guest resume · layout fixes ===== */
(()=>{
  ...
  function displayPerson1203(item){
    ...
    return role==='guest'?name.replace(/\s*#\d{3,8}\s*$/,''):name;
  }
  ...
  function ensureFestivalAdminQuickToggle1203(){document.querySelector('#festivalAdminQuickToggleV1203')?.remove()}
  ...
  async function refreshMyExperience1203({force=false,render=true}={}){
    ...
    try{
      ...
      return payload;
    }catch(error){console.warn('v12.05 my overview refresh failed',error);return null}finally{myRefreshBusy1203=false}
  }
...
/* ===== v12.05 guest account security · queue/push reliability · admin interaction fixes ===== */
(()=>{
  'use strict';
  if(window.__BAEMOON_FEATURES_1204__)return;
  window.__BAEMOON_FEATURES_1204__=true;
  const API_BASE='https://baemoon-reservation-server.timon20091220.workers.dev';
  const GUEST_LOCK_KEY='bm_guest_account_lock_v1';
  const NOTIFY_KEY='bm_notify_default_v1202';
  const DEVICE_KEY='bm_push_device_id_v1202';
  const SESSION_KEY='bm_push_session_id_v1202';
  const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]));
  const safeParticipantName=item=>{const name=String(item?.display_name||item?.displayName||'참여자');return String(item?.participant_role||item?.participantRole)==='guest'?name.replace(/\s*#\d{3,8}\s*$/,''):name};
  let modulesPromise=null,firestorePromise=null,uiTimer=0,reviewItems=[],reviewBusy=false,reviewLast=0,pushBusy=false;
  const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));
  function randomId(prefix){return `${prefix}-${crypto.randomUUID?.()||Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`}
  function localId(key,prefix,session=false){const store=session?sessionStorage:localStorage;let value='';try{value=store.getItem(key)||'';if(!value){value=randomId(prefix);store.setItem(key,value)}}catch{value=randomId(prefix)}return value}
  function guestLock(){try{return JSON.parse(localStorage.getItem(GUEST_LOCK_KEY)||'null')}catch{return null}}
  async function modules(){if(!modulesPromise)modulesPromise=Promise.all([import('https://www.gstatic.com/firebasejs/12.16.0/firebase-app.js'),import('https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js')]).then(([app,auth])=>({app,auth}));return modulesPromise}
  async function firestoreModule(){if(!firestorePromise)firestorePromise=import('https://www.gstatic.com/firebasejs/12.16.0/firebase-firestore.js');return firestorePromise}
  async function currentUser(){const {app,auth}=await modules();return auth.getAuth(app.getApp()).currentUser}
  async function api(path,{method='GET',body,retry=true}={}){
    const user=await currentUser();if(!user)throw new Error('로그인이 필요합니다.');const token=await user.getIdToken(false);
    let response;try{response=await fetch(`${API_BASE}${path}`,{method,cache:'no-store',headers:{Authorization:`Bearer ${token}`,Accept:'application/json',...(body===undefined?{}:{'Content-Type':'application/json'})},body:body===undefined?undefined:JSON.stringify(body)})}catch(error){if(retry){await sleep(300);return api(path,{method,body,retry:false})}throw new Error('예약 서버에 연결할 수 없습니다.')}
    let payload={};try{payload=await response.json()}catch{}
    if(response.status===401&&retry){await user.getIdToken(true);return api(path,{method,body,retry:false})}
    if(!response.ok||payload?.ok===false){const error=new Error(payload?.error?.message||`예약 서버 오류 (${response.status})`);error.code=payload?.error?.code;throw error}return payload;
  }

  function ensureGuestPasswordUi(){
    const modal=document.querySelector('#guestConfirmModal'),card=modal?.querySelector('.modal-card');if(!card)return;
    let wrap=card.querySelector('#guestPasswordAreaV1205');
    if(!wrap){
      wrap=document.createElement('div');wrap.id='guestPasswordAreaV1205';wrap.className='guest-password-area-v1205';
      wrap.innerHTML='<label class="guest-password-field-v1205"><span>비밀번호</span><input id="guestPasswordV1205" type="password" autocomplete="current-password" placeholder="최초 이용자는 baemoon2026"></label><small id="guestAccountRuleV1205">처음 이용할 때는 공통 비밀번호 baemoon2026을 입력한 뒤 개인 비밀번호를 설정합니다. 한 브라우저에서는 하나의 게스트 계정만 사용할 수 있습니다.</small>';
      const error=card.querySelector('#guestLoginError');(error||card.querySelector('#confirmGuestEntry'))?.insertAdjacentElement('beforebegin',wrap);
      wrap.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();card.querySelector('#confirmGuestEntry')?.click()}});
    }
    const lock=guestLock(),school=card.querySelector('#guestSchool'),name=card.querySelector('#guestName'),first=card.querySelector('#guestFirstPasswordV1205'),notice=card.querySelector('#guestAccountRuleV1205'),button=card.querySelector('#confirmGuestEntry'),keep=card.querySelector('.keep-login-v1202');
    if(keep)keep.hidden=true;
    if(lock){if(school){school.value=lock.school||'';school.readOnly=true}if(name){name.value=lock.name||'';name.readOnly=true}if(first)first.hidden=Boolean(lock.authEmail||lock.hasLegacyPassword);if(notice)notice.textContent=lock.authEmail?`이 브라우저의 기존 게스트 계정(${lock.school||''} ${lock.name||''})으로 로그인합니다.`:lock.hasLegacyPassword?'기존에 설정한 개인 비밀번호를 입력하면 Firebase 사용자로 전환됩니다.':`기존 게스트 계정을 Firebase 비밀번호 계정으로 전환합니다. 최초 비밀번호 baemoon2026을 입력한 뒤 개인 비밀번호를 설정해주세요.`;if(button)button.textContent='게스트 로그인'}
    else{if(school)school.readOnly=false;if(name)name.readOnly=false;if(first)first.hidden=false;if(notice)notice.textContent='첫 비밀번호는 baemoon2026입니다. 최초 등록 후 이 브라우저에서는 다른 게스트 계정을 만들 수 없습니다.'}
  }
  function hideGuestCodes(){
    document.querySelectorAll('#guestCode,[data-guest-code],.guest-code').forEach(node=>node.hidden=true);
    const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const changes=[];let node;
    while((node=walker.nextNode())){const value=node.nodeValue||'';if(/동명이인\s*확인\s*(번호|코드)/.test(value))changes.push([node,value.replace(/\s*[·|]?\s*동명이인\s*확인\s*(번호|코드)\s*#?[A-Za-z0-9_-]*/g,'').trim()]);else if(/게스트/.test(node.parentElement?.closest?.('article,li,button,section')?.textContent||'')&&/\s#\d{3,8}\b/.test(value))changes.push([node,value.replace(/\s#\d{3,8}\b/g,'')])}
    changes.forEach(([node,value])=>{node.nodeValue=value});
  }
  function cleanupAdminToggle(){document.querySelectorAll('#festivalAdminQuickToggleV1203,.festival-admin-quick-toggle-v1203').forEach(button=>button.remove())}

  function normalizeReservationV1204(row){return {id:String(row.id||''),requestId:String(row.request_id||row.requestId||''),slotId:String(row.slot_key||row.slotId||''),festivalId:String(row.festival_id||row.festivalId||''),festivalName:String(row.festival_name||row.festivalName||''),boothId:String(row.booth_id||row.boothId||''),boothName:String(row.booth_name||row.boothName||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||row.userUid||''),participantRole:String(row.participant_role||row.participantRole||''),user:String(row.display_name||row.displayName||row.user||'예약자'),name:String(row.display_name||row.displayName||row.name||'예약자'),guestCode:'',groupSize:Number(row.group_size||row.groupSize||1),status:String(row.status||'confirmed'),checkInCode:String(row.check_in_code||row.checkInCode||''),checkedIn:Boolean(Number(row.checked_in||row.checkedIn||0)),checkedInAt:Number(row.checked_in_at||row.checkedInAt||0)||null,createdAt:Number(row.created_at||row.createdAt||Date.now()),updatedAt:Number(row.updated_at||row.updatedAt||Date.now())}}
  function normalizeQueueV1204(row){return {id:String(row.id||''),slotId:String(row.slot_key||row.slotId||''),festivalId:String(row.festival_id||row.festivalId||''),festivalName:String(row.festival_name||row.festivalName||''),boothId:String(row.booth_id||row.boothId||''),boothName:String(row.booth_name||row.boothName||''),time:String(row.time||'즉시 예약'),userUid:String(row.user_uid||row.userUid||''),participantRole:String(row.participant_role||row.participantRole||''),displayName:String(row.display_name||row.displayName||'예약자'),guestCode:'',queueNumber:Number(row.queue_number||row.queueNumber||0),status:String(row.status||'waiting'),callCount:Number(row.call_count||row.callCount||0),aheadCount:Number(row.aheadCount||row.ahead_count||0),createdAt:Number(row.created_at||row.createdAt||Date.now()),updatedAt:Number(row.updated_at||row.updatedAt||Date.now())}}
  async function openGuestQueueQr(queueId){
    const payload=await api('/api/me/overview');const queues=(payload.queue||[]).map(normalizeQueueV1204),reservations=(payload.reservations||[]).map(normalizeReservationV1204);const queue=queues.find(item=>item.id===String(queueId));
    if(!queue)throw new Error('현재 대기번호를 찾지 못했습니다. 화면을 새로고침한 뒤 다시 시도해주세요.');
    const reservation=reservations.find(item=>item.slotId&&item.slotId===queue.slotId)||reservations.find(item=>item.festivalId===queue.festivalId&&item.boothId===queue.boothId&&String(item.time)===String(queue.time))||reservations.find(item=>item.boothId===queue.boothId);
    if(!reservation)throw new Error('QR에 연결된 예약을 찾지 못했습니다. 관리자에게 대기번호를 확인해주세요.');
    window.firebaseCache.reservations=reservations;window.firebaseCache.queueEntries=queues;window.firebaseCache.participations=payload.participations||[];window.firebaseCache.reviews=payload.reviews||[];
    if(window.baemoonApp?.state)window.baemoonApp.state.currentReservationId=reservation.id;document.querySelector('#openReservationQrButton')?.click();
  }
  window.addEventListener('click',event=>{const button=event.target.closest?.('[data-show-queue-qr-v1143]');if(!button)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();button.disabled=true;openGuestQueueQr(button.dataset.showQueueQrV1143).catch(error=>window.baemoonApp?.toast?.(error.message||'QR을 열지 못했습니다.')).finally(()=>button.disabled=false)},true);

  function improveQueueCards(){document.querySelectorAll('#adminBoothQueueList .admin-queue-student-v1143').forEach(card=>card.classList.add('admin-queue-student-v1204'))}
  function improveRosterButtons(){
    document.querySelectorAll('[data-admin-roster-v1203]').forEach(card=>{card.classList.add('admin-stat-action-v1204');let cue=card.querySelector('.admin-stat-cue-v1204,.admin-stat-cue-v1203');if(!cue){cue=document.createElement('span');cue.className='admin-stat-cue-v1204';cue.innerHTML='<span aria-hidden="true">☰</span><b>명단 보기</b>';card.append(cue);return}if(!cue.classList.contains('admin-stat-cue-v1204'))cue.className='admin-stat-cue-v1204';if(!cue.querySelector('b')||cue.querySelector('b')?.textContent!=='명단 보기')cue.innerHTML='<span aria-hidden="true">☰</span><b>명단 보기</b>'});
  }

  function ensureAdminReviewModal(){let modal=document.querySelector('#adminReviewModalV1204');if(modal)return modal;modal=document.createElement('div');modal.id='adminReviewModalV1204';modal.className='admin-review-modal-v1204';modal.hidden=true;modal.innerHTML='<section role="dialog" aria-modal="true"><button type="button" class="admin-review-close-v1204" data-close-admin-review-v1204>×</button><p class="eyebrow dark">REVIEW</p><h2 id="adminReviewNameV1204">후기</h2><p id="adminReviewMetaV1204"></p><div id="adminReviewStarsV1204"></div><div id="adminReviewTextV1204"></div><footer><button type="button" data-close-admin-review-v1204>닫기</button></footer></section>';document.body.append(modal);modal.addEventListener('click',event=>{if(event.target===modal||event.target.closest('[data-close-admin-review-v1204]')){modal.classList.remove('open');setTimeout(()=>modal.hidden=true,150)}});return modal}
  async function renderAdminReviewsV1204(force=false){
    if(window.baemoonApp?.session?.()?.role!=='admin'||reviewBusy)return;let section=document.querySelector('#adminReviewsV1136');
    if(!section){const wrap=document.querySelector('#festivalAdminDetailV1136');if(!wrap)return;section=document.createElement('section');section.id='adminReviewsV1136';section.className='admin-reviews-v1136';section.innerHTML='<div class="admin-participation-head"><div><b>학생·게스트 후기</b><small>이름을 누르면 별점과 후기를 확인합니다.</small></div><button type="button" id="refreshAdminReviewsV1204">새로고침</button></div><div id="adminReviewListV1136"></div>';wrap.append(section)}
    const list=section.querySelector('#adminReviewListV1136');if(!list)return;
    const correctlyRendered=reviewItems.length?Boolean(list.querySelector('.admin-review-tile-v1204')):Boolean(list.querySelector('.admin-empty'));
    if(!force&&Date.now()-reviewLast<10000&&correctlyRendered)return;reviewBusy=true;if(force)list.innerHTML='<div class="admin-empty">후기를 불러오는 중…</div>';
    try{const result=await api('/api/admin/reviews');reviewLast=Date.now();reviewItems=result.reviews||[];list.innerHTML=reviewItems.length?reviewItems.map(item=>`<button type="button" class="admin-review-tile-v1204" data-admin-review-v1204="${esc(item.id)}"><b>${esc(safeParticipantName(item))}</b><small>${esc(item.booth_name||'부스')}</small></button>`).join(''):'<div class="admin-empty">등록된 후기가 없습니다.</div>';section.hidden=false}catch(error){list.innerHTML=`<div class="admin-empty">${esc(error.message||'후기를 불러오지 못했습니다.')}</div>`}finally{reviewBusy=false}
  }
  window.addEventListener('click',event=>{
    const card=event.target.closest?.('[data-admin-review-v1204]');if(card){event.preventDefault();event.stopPropagation();const item=reviewItems.find(row=>String(row.id)===String(card.dataset.adminReviewV1204));if(!item)return;const modal=ensureAdminReviewModal();modal.querySelector('#adminReviewNameV1204').textContent=safeParticipantName(item);modal.querySelector('#adminReviewMetaV1204').textContent=`${item.festival_name||'행사'} · ${item.booth_name||'부스'}`;modal.querySelector('#adminReviewStarsV1204').textContent=`${'★'.repeat(Math.max(0,Number(item.rating)||0))}${'☆'.repeat(Math.max(0,5-(Number(item.rating)||0)))} · ${Number(item.rating||0).toFixed(1)}점`;modal.querySelector('#adminReviewTextV1204').textContent=item.review_text||'';modal.hidden=false;requestAnimationFrame(()=>modal.classList.add('open'));return}
    if(event.target.closest?.('#refreshAdminReviewsV1204,#refreshAdminReviewsV1136')){setTimeout(()=>renderAdminReviewsV1204(true),0)}
  },true);

  // 행사 관리 탭은 재배치 후에도 항상 직접 전환되도록 처리합니다.
  window.addEventListener('click',event=>{const button=event.target.closest?.('#managerTabs button[data-manager-tab]');if(!button)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();const tabs=button.closest('#managerTabs');tabs.querySelectorAll('button[data-manager-tab]').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('.manager-panel[data-manager-panel]').forEach(panel=>{const active=String(panel.dataset.managerPanel)===String(button.dataset.managerTab);panel.classList.toggle('active',active);panel.hidden=!active})},true);

  async function writeNoResponseNotice(queue,message){
    const uid=String(queue?.userUid||queue?.user_uid||'');if(!uid)return;
    try{const {app}=await modules(),firestore=await firestoreModule(),db=firestore.getFirestore(app.getApp()),ref=firestore.doc(firestore.collection(db,'userNotifications',uid,'items'));await firestore.setDoc(ref,{id:ref.id,title:'호출 무응답 처리 안내',body:message,type:'queue-no-response',audience:'개인',ownerUid:uid,queueId:String(queue?.id||''),boothName:String(queue?.boothName||queue?.booth_name||'부스'),createdAt:Date.now()})}catch(error){console.warn('무응답 안내 저장 실패',error)}
  }
  window.addEventListener('click',event=>{const button=event.target.closest?.('[data-skip-queue]');if(!button)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();(async()=>{button.disabled=true;const id=String(button.dataset.skipQueue||''),queue=(window.firebaseCache?.queueEntries||[]).map(normalizeQueueV1204).find(item=>item.id===id);try{const result=await api('/api/admin/queue/skip',{method:'POST',body:{queueId:id}});if(queue)await writeNoResponseNotice(queue,result.message||`${queue.boothName||'부스'} 부스 호출에 응답하지 않아 예약과 대기번호가 취소되었습니다.`);if(window.firebaseCache?.queueEntries)window.firebaseCache.queueEntries=window.firebaseCache.queueEntries.filter(item=>String(item.id)!==id);if(typeof loadQueueTime==='function')await loadQueueTime(typeof selectedQueueTime==='function'?selectedQueueTime():'즉시 예약');await window.baemoonFirebase?.getAdminStats?.({force:true}).catch(()=>{});window.baemoonApp?.toast?.('무응답 처리를 완료했고 학생 화면에 안내를 보냈습니다.')}catch(error){window.baemoonApp?.toast?.(error.message||'무응답 처리에 실패했습니다.')}finally{button.disabled=false}})()},true);

  function bytes(value){const pad='='.repeat((4-value.length%4)%4),base64=(value+pad).replace(/-/g,'+').replace(/_/g,'/');return Uint8Array.from(atob(base64),char=>char.charCodeAt(0))}
  async function registration(){if(!('serviceWorker'in navigator))throw new Error('이 브라우저는 앱 알림을 지원하지 않습니다.');return (await window.baemoonApp?.ensureServiceWorker?.())||navigator.serviceWorker.ready}
  function ensurePermissionGuide(){let modal=document.querySelector('#notificationPermissionGuideV1204');if(modal)return modal;modal=document.createElement('div');modal.id='notificationPermissionGuideV1204';modal.className='notification-guide-v1204';modal.hidden=true;modal.innerHTML='<section role="dialog" aria-modal="true"><button type="button" data-close-notification-guide-v1204>×</button><p class="eyebrow dark">APP NOTIFICATION</p><h2>알림 권한을 허용해주세요</h2><p>브라우저에서 알림을 차단한 경우 사이트가 권한창을 다시 강제로 열 수 없습니다.</p><ol><li><b>Android Chrome</b>: 주소창 왼쪽 사이트 설정 → 권한 → 알림 → 허용</li><li><b>휴대전화 설정</b>: 설정 → 알림 → 사용 중인 브라우저 → 알림 허용</li><li><b>iPhone</b>: Safari에서 홈 화면에 추가한 뒤 앱을 열어 알림을 켜주세요.</li></ol><footer><button type="button" data-close-notification-guide-v1204>확인</button></footer></section>';document.body.append(modal);modal.addEventListener('click',event=>{if(event.target===modal||event.target.closest('[data-close-notification-guide-v1204]')){modal.classList.remove('open');setTimeout(()=>modal.hidden=true,120)}});return modal}
  function openPermissionGuide(){const modal=ensurePermissionGuide();modal.hidden=false;requestAnimationFrame(()=>modal.classList.add('open'))}
  async function enablePush({silent=false}={}){
    if(pushBusy)return;pushBusy=true;const toggle=document.querySelector('#notificationToggle');if(toggle)toggle.disabled=true;
    try{
      if(!window.isSecureContext||!('Notification'in window)||!('PushManager'in window))throw new Error('현재 브라우저에서는 사이트 밖 알림을 지원하지 않습니다. Chrome 또는 홈 화면 앱으로 접속해주세요.');
      if(Notification.permission==='denied'){openPermissionGuide();throw new Error('알림 권한이 차단되어 있습니다. 안내에 따라 브라우저 또는 휴대전화 설정에서 허용해주세요.');}
      if(Notification.permission!=='granted'){const permission=await Notification.requestPermission();if(permission!=='granted'){openPermissionGuide();throw new Error('알림 권한을 허용해야 호출 알림을 받을 수 있습니다.')}}
      const reg=await registration();await reg.update().catch(()=>{});let subscription=await reg.pushManager.getSubscription();
      if(!subscription){const key=await api('/api/push/public-key');if(!key.publicKey)throw new Error('알림 서버 키가 설정되지 않았습니다.');subscription=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:bytes(key.publicKey)})}
      const deviceId=localId(DEVICE_KEY,'device'),sessionId=localId(SESSION_KEY,'session',true);await api('/api/push/subscribe',{method:'POST',body:{subscription:subscription.toJSON(),deviceId,sessionId,userAgent:navigator.userAgent}});await api('/api/push/session',{method:'POST',body:{endpoint:subscription.endpoint,deviceId,sessionId}});await window.baemoonFirebase?.setNotificationPreference?.(true).catch(()=>{});localStorage.setItem(NOTIFY_KEY,'1');if(toggle)toggle.checked=true;if(!silent)window.baemoonApp?.toast?.('앱 알림을 켰습니다. 사이트를 닫아도 지원되는 브라우저에서는 호출 알림이 표시됩니다.');
    }catch(error){if(toggle)toggle.checked=false;localStorage.setItem(NOTIFY_KEY,'0');if(!silent)window.baemoonApp?.toast?.(error.message||'알림을 켜지 못했습니다.')}finally{if(toggle)toggle.disabled=false;pushBusy=false;runUi()}
  }
  async function disablePush(){
    if(pushBusy)return;pushBusy=true;const toggle=document.querySelector('#notificationToggle');if(toggle)toggle.disabled=true;
    try{let endpoint='';try{endpoint=(await (await registration()).pushManager.getSubscription())?.endpoint||''}catch{}await api('/api/push/deactivate',{method:'POST',body:{endpoint,deviceId:localId(DEVICE_KEY,'device'),sessionId:localId(SESSION_KEY,'session',true)}}).catch(()=>{});await window.baemoonFirebase?.setNotificationPreference?.(false).catch(()=>{});localStorage.setItem(NOTIFY_KEY,'0');if(toggle)toggle.checked=false;window.baemoonApp?.toast?.('앱 알림을 껐습니다. 스위치를 다시 누르면 재연결할 수 있습니다.')}finally{if(toggle)toggle.disabled=false;pushBusy=false;runUi()}
  }
  window.__toggleNotificationsV1204=enabled=>enabled?enablePush({silent:false}):disablePush();
  async function repairPush(){if(window.__toggleNotificationsV1205){if(localStorage.getItem(NOTIFY_KEY)!=='0'&&'Notification'in window&&Notification.permission==='granted')await window.__toggleNotificationsV1205(true);return}if(localStorage.getItem(NOTIFY_KEY)==='0'||!window.baemoonApp?.session?.()||!('Notification'in window)||Notification.permission!=='granted')return;try{await enablePush({silent:true})}catch{}}

  function showNoResponse(payload){const key=String(payload?.messageId||payload?.queueId||'');if(key&&sessionStorage.getItem(`bm-no-response-${key}`))return;if(key)sessionStorage.setItem(`bm-no-response-${key}`,'1');const message=payload?.body||'호출에 응답하지 않아 예약과 대기번호가 취소되었습니다.';if(window.__bmAppAlertV1130)window.__bmAppAlertV1130('호출 무응답 처리 안내',message);else window.baemoonApp?.toast?.(message)}
  navigator.serviceWorker?.addEventListener?.('message',event=>{const message=event.data||{},payload=message.payload||{};if(message.type==='PUSH_RECEIVED'&&payload.type==='queue-no-response')showNoResponse(payload)});
  window.addEventListener('baemoon:push-received',event=>{if(event.detail?.type==='queue-no-response')showNoResponse(event.detail)});

  function runUi(){
    ensureGuestPasswordUi();hideGuestCodes();cleanupAdminToggle();improveQueueCards();improveRosterButtons();
    document.documentElement.dataset.baemoonVersion='13.01';
    const toggle=document.querySelector('#notificationToggle');if(toggle){if(!toggle.dataset.v1204)toggle.dataset.v1204='1';const permissionGranted='Notification'in window&&Notification.permission==='granted';toggle.checked=localStorage.getItem(NOTIFY_KEY)!=='0'&&permissionGranted;const row=toggle.closest('.notification-settings')||toggle.parentElement;if(row&&!document.querySelector('#notificationPermissionHintV1204')){const hint=document.createElement('small');hint.id='notificationPermissionHintV1204';hint.className='notification-permission-hint-v1204';hint.textContent=permissionGranted?'브라우저 알림 권한이 허용되어 있습니다.':'스위치를 켜면 브라우저의 알림 권한 요청이 표시됩니다.';row.insertAdjacentElement('afterend',hint)}else{const hint=document.querySelector('#notificationPermissionHintV1204'),message=permissionGranted?'브라우저 알림 권한이 허용되어 있습니다.':'스위치를 켜면 브라우저의 알림 권한 요청이 표시됩니다.';if(hint&&hint.textContent!==message)hint.textContent=message}}
    document.querySelectorAll('#testNotificationV1202,#diagnoseNotificationV1202,#notificationDiagnosticV1202,#testBackgroundPushV1143').forEach(node=>node.remove());
    if(window.baemoonApp?.session?.()?.role==='admin'&&window.baemoonApp?.state?.screen==='festival')renderAdminReviewsV1204(false);
  }
  window.__bmUiV1204=runUi;
  window.addEventListener('pageshow',()=>{runUi();setTimeout(repairPush,800)});window.addEventListener('online',()=>setTimeout(repairPush,400));document.addEventListener('visibilitychange',()=>{if(!document.hidden){runUi();setTimeout(repairPush,500)}});
  setTimeout(()=>{runUi();setTimeout(repairPush,1000)},0);
  console.info('Baemoon v12.05 features ready');
})();


/* ===== v12.05 Firebase guest auth · notification recovery · compact admin rosters ===== */
(()=>{
  'use strict';
  if(window.__BAEMOON_FEATURES_1205__)return;
  window.__BAEMOON_FEATURES_1205__=true;
  const API='https://baemoon-reservation-server.timon20091220.workers.dev';
  const DEVICE_KEY='bm_push_device_id_v1202',SESSION_KEY='bm_push_session_id_v1202',NOTIFY_KEY='bm_notify_enabled_v1204';
  let busy=false,lastUid='',pollBusy=false,lastQueueState=new Map(),timer=0;
  const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));
  function rid(prefix){return `${prefix}-${crypto.randomUUID?.()||Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`}
  function storedId(key,prefix,session=false){const store=session?sessionStorage:localStorage;let value='';try{value=store.getItem(key)||'';if(!value){value=rid(prefix);store.setItem(key,value)}}catch{value=rid(prefix)}return value}
  async function authUser(){const [app,auth]=await Promise.all([import('https://www.gstatic.com/firebasejs/12.16.0/firebase-app.js'),import('https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js')]);return auth.getAuth(app.getApp()).currentUser}
  async function request(path,{method='GET',body,retry=true}={}){const user=await authUser();if(!user)throw new Error('로그인이 필요합니다.');const token=await user.getIdToken(false);let response;try{response=await fetch(API+path,{method,cache:'no-store',headers:{Authorization:`Bearer ${token}`,Accept:'application/json',...(body===undefined?{}:{'Content-Type':'application/json'})},body:body===undefined?undefined:JSON.stringify(body)})}catch(error){if(retry){await sleep(300);return request(path,{method,body,retry:false})}throw new Error('알림 서버에 연결할 수 없습니다.')}let payload={};try{payload=await response.json()}catch{}if(response.status===401&&retry){await user.getIdToken(true);return request(path,{method,body,retry:false})}if(!response.ok||payload?.ok===false)throw new Error(payload?.error?.message||`서버 오류 (${response.status})`);return payload}
  function bytes(value){const pad='='.repeat((4-value.length%4)%4),base64=(value+pad).replace(/-/g,'+').replace(/_/g,'/');return Uint8Array.from(atob(base64),c=>c.charCodeAt(0))}
  async function registration(){if(!('serviceWorker'in navigator))throw new Error('현재 브라우저는 앱 알림을 지원하지 않습니다.');return (await window.baemoonApp?.ensureServiceWorker?.())||navigator.serviceWorker.ready}
  function permissionHelp(){let modal=document.querySelector('#notificationPermissionGuideV1205');if(!modal){modal=document.createElement('div');modal.id='notificationPermissionGuideV1205';modal.className='notification-guide-v1204';modal.hidden=true;modal.innerHTML='<section role="dialog" aria-modal="true"><button type="button" data-close-notification-guide-v1205>×</button><p class="eyebrow dark">APP NOTIFICATION</p><h2>알림 권한을 허용해주세요</h2><p>브라우저가 이미 알림을 차단한 경우 사이트 코드로 강제로 바꿀 수 없습니다.</p><ol><li><b>Android Chrome</b>: 주소창 왼쪽 사이트 설정 → 알림 → 허용</li><li><b>휴대전화 설정</b>: 설정 → 알림 → Chrome 또는 사용 중인 브라우저 → 허용</li><li><b>iPhone</b>: Safari에서 홈 화면에 추가한 배문고 앱으로 실행한 뒤 알림을 켜주세요.</li><li><b>카카오톡</b>: 오른쪽 위 메뉴에서 Chrome 또는 Safari로 열어주세요.</li></ol><footer><button type="button" data-close-notification-guide-v1205>확인</button></footer></section>';document.body.append(modal);modal.addEventListener('click',e=>{if(e.target===modal||e.target.closest('[data-close-notification-guide-v1205]')){modal.classList.remove('open');setTimeout(()=>modal.hidden=true,120)}})}modal.hidden=false;requestAnimationFrame(()=>modal.classList.add('open'))}
  async function syncPush({ask=false,toast=false}={}){
    if(busy)return false;busy=true;const toggle=document.querySelector('#notificationToggle');if(toggle)toggle.disabled=true;
    try{
      if(!window.isSecureContext||!('Notification'in window)||!('PushManager'in window)){if(ask)permissionHelp();throw new Error('이 접속 환경은 Web Push를 지원하지 않습니다. Chrome·Safari 홈 화면 앱으로 열어주세요.')}
      if(Notification.permission==='denied'){if(ask)permissionHelp();throw new Error('알림 권한이 차단되어 있습니다. 안내에 따라 브라우저 또는 휴대전화 설정에서 허용해주세요.')}
      if(Notification.permission!=='granted'){
        if(!ask)return false;
        const result=await Notification.requestPermission();if(result!=='granted'){permissionHelp();throw new Error('알림 권한을 허용해야 호출 알림을 받을 수 있습니다.')}
      }
      const user=await authUser();if(!user)return false;
      const reg=await registration();await reg.update().catch(()=>{});let subscription=await reg.pushManager.getSubscription();
      if(!subscription){const key=await request('/api/push/public-key');if(!key.publicKey)throw new Error('서버 알림 키가 설정되지 않았습니다.');subscription=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:bytes(key.publicKey)})}
      const deviceId=storedId(DEVICE_KEY,'device'),sessionId=storedId(SESSION_KEY,'session',true);
      await request('/api/push/subscribe',{method:'POST',body:{subscription:subscription.toJSON(),deviceId,sessionId,userAgent:navigator.userAgent}});
      await request('/api/push/session',{method:'POST',body:{endpoint:subscription.endpoint,deviceId,sessionId}});
      localStorage.setItem(NOTIFY_KEY,'1');localStorage.setItem('bm_notify_default_v1202','1');
      await window.baemoonFirebase?.setNotificationPreference?.(true).catch(()=>{});
      if(toggle){toggle.checked=true;toggle.dataset.pushReady='1'}
      if(toast)window.baemoonApp?.toast?.('앱 알림을 켰습니다.');return true;
    }catch(error){if(toggle)toggle.checked=false;if(toast)window.baemoonApp?.toast?.(error.message||'알림을 켜지 못했습니다.');return false}
    finally{if(toggle)toggle.disabled=false;busy=false;updateHint()}
  }
  async function turnOff(){if(busy)return;busy=true;const toggle=document.querySelector('#notificationToggle');if(toggle)toggle.disabled=true;try{let endpoint='';try{endpoint=(await (await registration()).pushManager.getSubscription())?.endpoint||''}catch{}await request('/api/push/deactivate',{method:'POST',body:{endpoint,deviceId:storedId(DEVICE_KEY,'device'),sessionId:storedId(SESSION_KEY,'session',true)}}).catch(()=>{});localStorage.setItem(NOTIFY_KEY,'0');localStorage.setItem('bm_notify_default_v1202','0');await window.baemoonFirebase?.setNotificationPreference?.(false).catch(()=>{});if(toggle)toggle.checked=false;window.baemoonApp?.toast?.('앱 알림을 껐습니다. 다시 켜면 같은 기기 구독을 복구합니다.')}finally{if(toggle)toggle.disabled=false;busy=false;updateHint()}}
  function updateHint(){const toggle=document.querySelector('#notificationToggle');if(!toggle)return;const enabled=localStorage.getItem(NOTIFY_KEY)!=='0',granted='Notification'in window&&Notification.permission==='granted';toggle.checked=enabled&&granted;let hint=document.querySelector('#notificationPermissionHintV1204,#notificationPermissionHintV1205');const row=toggle.closest('.notification-settings')||toggle.parentElement;if(!hint&&row){hint=document.createElement('small');hint.id='notificationPermissionHintV1205';hint.className='notification-permission-hint-v1204';row.insertAdjacentElement('afterend',hint)}if(hint)hint.textContent=granted?(enabled?'호출 알림이 이 기기에 연결되어 있습니다.':'앱 알림이 꺼져 있습니다. 스위치를 누르면 다시 연결됩니다.'):'스위치를 켜면 브라우저 알림 권한 요청이 표시됩니다.'}
  window.__handleNotificationToggleV1205=enabled=>enabled?syncPush({ask:true,toast:true}):turnOff();
  // 이전 버전의 자동 복구 호출은 권한창·토스트를 반복하지 않고 조용히 재연결합니다.
  window.__toggleNotificationsV1204=enabled=>enabled?syncPush({ask:false,toast:false}):turnOff();
  window.__toggleNotificationsV1205=window.__toggleNotificationsV1204;

  async function foregroundPoll(){const session=window.baemoonApp?.session?.();if(pollBusy||document.hidden||!session?.uid||!['student','guest'].includes(session.role))return;pollBusy=true;try{const data=await request('/api/me/overview');const next=new Map();for(const row of data.queue||[]){const id=String(row.id||''),status=String(row.status||'waiting'),count=Number(row.call_count||row.callCount||0);next.set(id,{status,count});const before=lastQueueState.get(id);if(before&&before.status!=='called'&&status==='called'){const body=`${row.booth_name||row.boothName||'부스'} 부스에서 호출입니다. QR을 준비하고 부스로 이동해주세요.`;window.baemoonApp?.toast?.(body)}}lastQueueState=next}catch{}finally{pollBusy=false}}
  function foregroundPushMessage(payload={}){
    const type=String(payload.type||''),key=String(payload.messageId||`${type}-${payload.queueId||payload.reservationId||''}`);
    if(key&&sessionStorage.getItem(`bm-push-visible-v1205-${key}`))return;
    if(key)sessionStorage.setItem(`bm-push-visible-v1205-${key}`,'1');
    const body=String(payload.body||'알림이 도착했습니다.');
    if(type==='queue-call')window.baemoonApp?.toast?.(body);
    else if(type==='queue-call-cancelled'||type==='queue-no-response'||type==='check-in'){
      if(window.__bmAppAlertV1130)window.__bmAppAlertV1130(String(payload.title||'배문고 알림'),body);
      else window.baemoonApp?.toast?.(body);
    }
    if(['queue-call','queue-call-cancelled','queue-no-response','check-in'].includes(type)){
      window.baemoonFirebase?.getMyServerOverview?.({force:true}).then(data=>{
        if(!data)return;
        window.firebaseCache.reservations=(data.reservations||[]).map(typeof normalizeReservation==='function'?normalizeReservation:value=>value);
        window.firebaseCache.queueEntries=(data.queue||[]).map(typeof normalizeQueue==='function'?normalizeQueue:value=>value);
        window.firebaseCache.participations=data.participations||[];
        window.baemoonApp?.renderHomeReservations?.();window.baemoonApp?.renderMy?.();
      }).catch(()=>{});
    }
  }
  navigator.serviceWorker?.addEventListener?.('message',event=>{if(event.data?.type==='PUSH_RECEIVED')foregroundPushMessage(event.data.payload||{})});
  window.addEventListener('baemoon:push-received',event=>foregroundPushMessage(event.detail||{}));

  async function authAndPushTick(){const session=window.baemoonApp?.session?.(),uid=String(session?.uid||'');if(uid&&uid!==lastUid){lastUid=uid;lastQueueState=new Map();if(localStorage.getItem(NOTIFY_KEY)!=='0'&&'Notification'in window&&Notification.permission==='granted')setTimeout(()=>syncPush({ask:false}),300)}else if(!uid){lastUid='';lastQueueState=new Map()}updateHint()}
  function removeLegacyUi(){document.querySelectorAll('#festivalAdminQuickToggleV1203,.festival-admin-quick-toggle-v1203,#testNotificationV1202,#diagnoseNotificationV1202,#notificationDiagnosticV1202,#testBackgroundPushV1143').forEach(n=>n.remove());document.documentElement.dataset.baemoonVersion='13.01'}
  function tick(){removeLegacyUi();authAndPushTick();foregroundPoll()}
  window.__bmUiV1205=()=>{removeLegacyUi();updateHint();authAndPushTick()};
  window.addEventListener('pageshow',()=>{tick();if(localStorage.getItem(NOTIFY_KEY)!=='0'&&'Notification'in window&&Notification.permission==='granted')setTimeout(()=>syncPush({ask:false}),500)});
  window.addEventListener('online',()=>setTimeout(()=>syncPush({ask:false}),500));document.addEventListener('visibilitychange',()=>{if(!document.hidden){tick();if(localStorage.getItem(NOTIFY_KEY)!=='0')syncPush({ask:false})}});
  setTimeout(tick,0);setInterval(()=>{if(!document.hidden)foregroundPoll()},8000);
})();


/* ===== 배문고 앱 v13.01 안정화 레이어 ===== */
(()=>{
  'use strict';
  if(window.__BAEMOON_V1301__)return;
  window.__BAEMOON_V1301__=true;
  const VERSION='13.01';
  const NOTIFY_KEY='bm_notifications_enabled_v1300';
  let pushBusy=false,lastPushUid='';

  window.ensureRebookingControlV1300=function ensureRebookingControlV1300(){
    if(document.querySelector('#boothAllowRebookingV1300'))return;
    const anchor=document.querySelector('#boothWaitlistEnabled')?.closest('label')||document.querySelector('#boothWaitlistEnabled')?.parentElement;
    if(!anchor)return;
    const label=document.createElement('label');
    label.className='booth-rebooking-field-v1300';
    label.innerHTML='<span>체크인 후 재예약</span><select id="boothAllowRebookingV1300"><option value="no">재예약 불가능</option><option value="yes">재예약 가능</option></select><small>진행 중인 예약·대기는 중복 신청할 수 없습니다.</small>';
    anchor.insertAdjacentElement('afterend',label);
  };

  function guestPasswordDialog(){
    let overlay=document.querySelector('#guestPersonalPasswordModalV1300');
    if(!overlay){
      overlay=document.createElement('div');overlay.id='guestPersonalPasswordModalV1300';overlay.className='guest-personal-password-modal-v1300';overlay.hidden=true;
      overlay.innerHTML='<section role="dialog" aria-modal="true" aria-labelledby="guestPersonalPasswordTitleV1300"><button type="button" class="guest-password-close-v1300" aria-label="닫기">×</button><p class="eyebrow dark">FIRST GUEST LOGIN</p><h2 id="guestPersonalPasswordTitleV1300">개인 비밀번호 설정</h2><p>다음 로그인부터 사용할 개인 비밀번호를 설정해주세요.</p><label><span>개인 비밀번호</span><input id="guestPersonalPasswordV1300" type="password" minlength="8" autocomplete="new-password" placeholder="8자 이상"></label><label><span>개인 비밀번호 확인</span><input id="guestPersonalPasswordConfirmV1300" type="password" minlength="8" autocomplete="new-password" placeholder="한 번 더 입력"></label><small id="guestPersonalPasswordErrorV1300" hidden></small><footer><button type="button" class="guest-password-cancel-v1300">취소</button><button type="button" class="guest-password-save-v1300">설정하고 입장</button></footer></section>';
      document.body.append(overlay);
    }
    return overlay;
  }
  window.baemoonRequestGuestPasswordV13=()=>new Promise(resolve=>{
    const overlay=guestPasswordDialog(),password=overlay.querySelector('#guestPersonalPasswordV1300'),confirm=overlay.querySelector('#guestPersonalPasswordConfirmV1300'),error=overlay.querySelector('#guestPersonalPasswordErrorV1300');
    password.value='';confirm.value='';error.hidden=true;overlay.hidden=false;requestAnimationFrame(()=>overlay.classList.add('open'));setTimeout(()=>password.focus(),50);
    let done=false;
    const finish=value=>{if(done)return;done=true;overlay.classList.remove('open');setTimeout(()=>overlay.hidden=true,150);cleanup();resolve(value)};
    const cleanup=()=>{overlay.querySelector('.guest-password-save-v1300').onclick=null;overlay.querySelector('.guest-password-cancel-v1300').onclick=null;overlay.querySelector('.guest-password-close-v1300').onclick=null;overlay.onclick=null};
    overlay.querySelector('.guest-password-save-v1300').onclick=()=>{
      const a=password.value,b=confirm.value;
      if(a.length<8){error.textContent='개인 비밀번호는 8자 이상이어야 합니다.';error.hidden=false;return}
      if(a==='baemoon2026'){error.textContent='최초 비밀번호와 다른 개인 비밀번호를 설정해주세요.';error.hidden=false;return}
      if(a!==b){error.textContent='개인 비밀번호 확인이 일치하지 않습니다.';error.hidden=false;return}
      finish(a);
    };
    overlay.querySelector('.guest-password-cancel-v1300').onclick=overlay.querySelector('.guest-password-close-v1300').onclick=()=>finish(null);
    overlay.onclick=e=>{if(e.target===overlay)finish(null)};
  });

  function normalizeGuestForm(){
    const card=document.querySelector('#guestConfirmModal .modal-card,#guestConfirmModal .auth-modal-card,#guestConfirmModal section,#guestConfirmModal');if(!card)return;
    const old=card.querySelector('#guestPasswordAreaV1205');
    if(old){
      old.querySelector('#guestFirstPasswordV1205')?.remove();
      const label=old.querySelector('.guest-password-field-v1205 span');if(label)label.textContent='비밀번호';
      const input=old.querySelector('#guestPasswordV1205');if(input)input.placeholder='최초 이용자는 baemoon2026';
      const notice=old.querySelector('#guestAccountRuleV1205');if(notice)notice.textContent='처음 이용할 때는 공통 비밀번호 baemoon2026을 입력한 뒤 개인 비밀번호를 설정합니다.';
    }
    const button=card.querySelector('#confirmGuestEntry');if(button)button.textContent='게스트 로그인';
  }

  function base64Bytes(value){const pad='='.repeat((4-value.length%4)%4),base64=(value+pad).replace(/-/g,'+').replace(/_/g,'/');return Uint8Array.from(atob(base64),c=>c.charCodeAt(0))}
  async function serviceWorkerRegistration(){if(!('serviceWorker'in navigator))throw new Error('현재 브라우저는 앱 알림을 지원하지 않습니다.');return (await window.baemoonApp?.ensureServiceWorker?.())||navigator.serviceWorker.ready}
  async function stablePushToggle(enabled,{ask=true,toast=true}={}){
    if(pushBusy)return false;pushBusy=true;const toggle=document.querySelector('#notificationToggle');if(toggle)toggle.disabled=true;
    try{
      if(!window.isSecureContext||!('Notification'in window)||!('PushManager'in window))throw new Error('이 브라우저에서는 Web Push를 사용할 수 없습니다. Chrome 또는 홈 화면 앱으로 열어주세요.');
      const api=window.baemoonFirebase,session=window.baemoonApp?.session?.();if(!api||!session?.uid)throw new Error('로그인 후 알림을 설정해주세요.');
      const registration=await serviceWorkerRegistration();let subscription=await registration.pushManager.getSubscription();
      if(!enabled){
        if(subscription){await api.removePushSubscription?.(subscription.endpoint).catch(()=>{});await subscription.unsubscribe().catch(()=>{})}
        localStorage.setItem(NOTIFY_KEY,'0');localStorage.setItem('bm_notify_default_v1202','0');localStorage.setItem('bm_notify_enabled_v1204','0');if(toggle)toggle.checked=false;
        if(toast)window.baemoonApp?.toast?.('앱 알림을 껐습니다.');return true;
      }
      if(Notification.permission==='denied')throw new Error('알림이 차단되어 있습니다. 브라우저 또는 휴대전화 설정에서 이 사이트의 알림을 허용해주세요.');
      if(Notification.permission!=='granted'){
        if(!ask)return false;const permission=await Notification.requestPermission();if(permission!=='granted')throw new Error('호출 알림을 받으려면 알림 권한을 허용해야 합니다.');
      }
      if(!subscription){const result=await api.getPushPublicKey();if(!result?.publicKey)throw new Error('서버의 알림 키가 설정되지 않았습니다.');subscription=await registration.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:base64Bytes(result.publicKey)})}
      await api.savePushSubscription(subscription.toJSON());
      localStorage.setItem(NOTIFY_KEY,'1');localStorage.setItem('bm_notify_default_v1202','1');localStorage.setItem('bm_notify_enabled_v1204','1');if(toggle){toggle.checked=true;toggle.dataset.pushReady='1'}
      if(toast)window.baemoonApp?.toast?.('앱 알림을 켰습니다.');return true;
    }catch(error){if(enabled&&toggle)toggle.checked=false;if(toast)window.baemoonApp?.toast?.(error?.message||'알림 설정에 실패했습니다.');return false}
    finally{if(toggle)toggle.disabled=false;pushBusy=false}
  }
  window.__handleNotificationToggleV1205=enabled=>stablePushToggle(Boolean(enabled),{ask:true,toast:true});
  window.__toggleNotificationsV1204=enabled=>stablePushToggle(Boolean(enabled),{ask:false,toast:false});
  window.__toggleNotificationsV1205=window.__toggleNotificationsV1204;
  window.baemoonStablePushV1300=stablePushToggle;

  function keepVersion(){
    document.documentElement.dataset.baemoonVersion=VERSION;
    document.querySelectorAll('[data-app-version],.app-version,.auth-welcome-card small').forEach(element=>{const text=element.textContent||'';if(/v(?:11|12)\.\d+/i.test(text))element.textContent=text.replace(/v(?:11|12)\.\d+/ig,'v13.01')});
  }
  function autoReconnect(){
    const session=window.baemoonApp?.session?.(),uid=String(session?.uid||'');
    if(uid&&uid!==lastPushUid){lastPushUid=uid;if(localStorage.getItem(NOTIFY_KEY)!=='0'&&'Notification'in window&&Notification.permission==='granted')setTimeout(()=>stablePushToggle(true,{ask:false,toast:false}),250)}
    if(!uid)lastPushUid='';
  }
  function tick(){normalizeGuestForm();window.ensureRebookingControlV1300();keepVersion();autoReconnect()}
  window.__bmUiV1301=tick;
  window.addEventListener('pageshow',tick);window.addEventListener('online',()=>setTimeout(autoReconnect,250));setTimeout(tick,0);
  console.info('Baemoon v13.01 stabilization layer ready');
})();


/* ===== v13.01 single UI coordinator · no whole-page MutationObserver ===== */
(()=>{
  'use strict';
  if(window.__BAEMOON_UI_COORDINATOR_1301__)return;
  window.__BAEMOON_UI_COORDINATOR_1301__=true;
  let scheduled=false,running=false,lastRun=0;
  const passes=['__bmUiV1133','__bmUiWordsV1135','__bmUiV1136','__bmUiV1142','__bmUiV1143','__bmUiV1144','__bmUiV1201','__bmUiV1202','__bmUiV1203','__bmUiV1204','__bmUiV1205','__bmUiV1301'];
  function run(){
    scheduled=false;
    if(running)return;
    const now=Date.now();
    if(now-lastRun<120){schedule(140);return}
    running=true;lastRun=now;
    try{
      for(const name of passes){
        const fn=window[name];
        if(typeof fn==='function'){try{fn()}catch(error){console.warn('[Baemoon UI pass]',name,error)}}
      }
  ...
  window.__baemoonScheduleUi1301=schedule;
  ...
  window.addEventListener('baemoon:data-ready',()=>schedule(0));
  window.addEventListener('baemoon:screen-changed',()=>schedule(0));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule(0)});
  setInterval(()=>{if(!document.hidden)schedule(0)},7000);
  schedule(0);
  console.info('Baemoon v13.01 single UI coordinator ready');
})();
