...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.13 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.13 Firestore 규칙을 게시해주세요.';
...
function listenNotifications(uid){
  listenerStops.push(onSnapshot(collection(db,"broadcastNotifications"),s=>{ui().handleBroadcastNotifications(s.docs.map(plain))},e=>ui().toast(displayFirebaseError(e))));
  listenerStops.push(onSnapshot(collection(db,"userNotifications",uid,"items"),s=>{ui().handlePersonalNotifications(s.docs.map(plain))},e=>ui().toast(displayFirebaseError(e))));
