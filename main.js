...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=11.7.0',{updateViaCache:'none'});
      await registration.update();
    }catch(error){console.error('Service worker registration failed:',error)}
  });
}

// ===== FIREBASE RUNTIME (runs only after UI exists) =====

if(!window.baemoonApp || typeof window.baemoonApp.setSession!=='function'){
  throw new Error('앱 UI 초기화 실패: setSession을 만들지 못했습니다.');
}
