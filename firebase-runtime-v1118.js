...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],notifications:[],
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.18 firestore.rules를 게시해주세요.';
  if(code.includes('permission-denied'))return '최신 v11.18 Firestore 규칙을 게시해주세요.';
...
function listenClassChat(role,profile){
  ...
  listenerStops.push(onSnapshot(
    ...
    snapshot=>{
      window.firebaseCache.classChatMessages=snapshot.docs.map(plain);
      ui().renderClassChat();
    },
    error=>ui().toast(displayFirebaseError(error))
  ));
...
window.baemoonFirebase={
  ...
  async saveTimetable(payload){
    ...
    const days=Array.isArray(payload.days)?payload.days.map(day=>
      Array.from({length:7},(_,index)=>String(day?.[index]||'').slice(0,30))
  ...
  async sendClassChatMessage(payload){
    ...
    const data={
      ...
      classNo:Number(current.classNo),
      authorUid:auth.currentUser.uid,
      authorKey:current.studentKey,
