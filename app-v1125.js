...
const state={
  ...
  classChatUnread:0,classChatInitialized:false,adminTimetableGrade:1,adminTimetableClass:1,adminTimetableDrafts:{},currentQueueBoothKey:null,qrScannerStream:null,qrScanActive:false
};
...
function ensureServiceWorker(){
  ...
  if(!serviceWorkerRegistrationPromise){
    serviceWorkerRegistrationPromise=navigator.serviceWorker
      .register('./sw-v1125.js',{scope:'./',updateViaCache:'none'})
...
/* v11.25 시간표 저장: 문서 캡처 단계에서 단 한 번만 처리 */
...
/* v11.25 부스 저장: 즉시 예약 허용 + 단일 저장 이벤트 */
document.addEventListener('click',async event=>{
...
openReservation=function(boothId){
  ...
  const shouldShowWait=booth.waitlistEnabled&&(status.closed||status.full||!firstAvailable||ownQueue);
  ...
  if(shouldShowWait){
    ...
    $('#joinBoothQueueButton').textContent=ownQueue
      ?'대기 취소'
      :'대기번호 받기';
    $('#joinBoothQueueButton').dataset.action=ownQueue?'cancel':'join';
  }
...
function renderAdminStats(){
  ...
  const reservedPeople=rs.reduce((sum,item)=>sum+Math.max(1,Number(item.groupSize||1)),0);
  const checkedPeople=rs.filter(item=>item.checkedIn)
    .reduce((sum,item)=>sum+Math.max(1,Number(item.groupSize||1)),0);
  const waiting=queueEntries().filter(item=>item.status==='waiting').length;
  const closed=allBooths().filter(item=>item.booth.bookingClosed).length;
  const rate=reservedPeople?Math.round(checkedPeople/reservedPeople*100):0;
  ...
  $('#statsWaitingCount').textContent=`${waiting}명`;
  $('#statsClosedBooths').textContent=`${closed}개`;

  const ranking=allBooths().map(({festival,booth})=>{
    const boothReservations=rs.filter(item=>
      item.festivalId===festival.id&&item.boothId===booth.id
    );
    const people=boothReservations.reduce((sum,item)=>
      sum+Math.max(1,Number(item.groupSize||1)),0);
    const checkins=boothReservations.filter(item=>item.checkedIn)
      .reduce((sum,item)=>sum+Math.max(1,Number(item.groupSize||1)),0);
    const queue=activeBoothQueue(festival.id,booth.id).length;
    return {festival,booth,people,checkins,queue};
  }).sort((a,b)=>b.people-a.people);

  $('#adminBoothRanking').innerHTML=ranking.length?ranking.map((item,index)=>`
    <article>
      <span>${index+1}</span>
      <div><b>${esc(item.booth.name)}</b>
      <small>${esc(item.festival.name)} · 예약 ${item.people}명 · 체크인 ${item.checkins}명 · 대기 ${item.queue}명</small></div>
      <i>${item.booth.bookingClosed?'마감':boothStatusInfo(item.festival,item.booth).congestion.label}</i>
    </article>`).join(''):'<div class="admin-empty">통계에 표시할 부스가 없습니다.</div>';
}
$('#refreshAdminStats').addEventListener('click',()=>{
  renderAdminStats();
  toast('통계를 갱신했습니다.');
});
...
openAdminBoothReservations=function(key){
  ...
  renderAdminBoothQueue(entry.festival.id,entry.booth.id);
};
...
function renderAdminBoothQueue(festivalId,boothId){
  ...
  $('#adminBoothQueueList').innerHTML=entries.length?entries.map((item,index)=>`
    <article class="${item.status}">
      ...
      <div>
        ${item.status==='waiting'?`<button data-call-queue="${item.id}">호출</button>`:''}
  ...
  $$('[data-call-queue]').forEach(button=>button.addEventListener('click',async()=>{
    try{await window.baemoonFirebase.updateQueueStatus(button.dataset.callQueue,'called')}
    catch(error){toast(firebaseFriendlyMessage(error))}
  }));
...
$('#callNextQueueButton').addEventListener('click',async()=>{
  ...
  try{
    await window.baemoonFirebase.updateQueueStatus(next.id,'called');
