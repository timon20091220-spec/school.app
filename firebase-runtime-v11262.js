...
window.firebaseCache={
  ...
  dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],queueEntries:[],adminReservationStats:null,reservationServerStatus:'checking',notifications:[],
...
function displayFirebaseError(error){
  ...
  if(code.includes('reservation-permission'))return '예약 권한 규칙을 적용하지 못했습니다. v11.26 firestore.rules를 게시해주세요.';
...
async function reservationServerRequest(path,{method='GET',body,forceToken=false,retry=true}={}){
  requireUser();
  ...
  try{
    ...
    wrapped.code='reservation-server-network';throw wrapped;
  }
  let payload={};
  try{payload=await response.json()}catch{}
  if(response.status===401&&retry){
    return reservationServerRequest(path,{method,body,forceToken:true,retry:false});
  }
...
async function refreshReservationServerData(role=ui().session()?.role){
  if(!auth.currentUser)return {reservations:[],queue:[]};
  try{
    if(role==='admin'){
      const [reservationsPayload,statsPayload]=await Promise.all([
        reservationServerRequest('/api/admin/reservations'),
        reservationServerRequest('/api/admin/stats')
      ]);
      window.firebaseCache.reservations=(reservationsPayload.reservations||[]).map(normalizeServerReservation);
      window.firebaseCache.adminReservationStats=statsPayload;
      window.firebaseCache.reservationServerStatus='connected';
      ui().renderAdminReservations();
      if(typeof ui().renderAdminStats==='function')ui().renderAdminStats();
      if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
      return {reservations:window.firebaseCache.reservations,stats:statsPayload};
    }
    if(['student','guest'].includes(role)){
      const [reservationsPayload,queuePayload]=await Promise.all([
        reservationServerRequest('/api/me/reservations'),
        reservationServerRequest('/api/me/queue')
      ]);
      window.firebaseCache.reservations=(reservationsPayload.reservations||[]).map(normalizeServerReservation);
      window.firebaseCache.queueEntries=(queuePayload.queue||[]).map(normalizeServerQueue);
      window.firebaseCache.reservationServerStatus='connected';
      ...
      return {reservations:window.firebaseCache.reservations,queue:window.firebaseCache.queueEntries};
    }
  }catch(error){
    window.firebaseCache.reservationServerStatus='error';
    if(typeof ui().renderReservationServerStatus==='function')ui().renderReservationServerStatus();
...
async function refreshServerSlotStatus(festivalId,boothId,time='즉시 예약'){
  const params=new URLSearchParams({festivalId:String(festivalId),boothId:String(boothId),time:String(time||'즉시 예약')});
  const payload=await reservationServerRequest(`/api/slot/status?${params}`);
...
function stopListeners(){reservationSlotSyncStarted=false;listenerStops.forEach(stop=>{try{stop()}catch{}});listenerStops=[];if(suggestionMessageStop){try{suggestionMessageStop()}catch{}suggestionMessageStop=null;}suggestionParts={public:[],own:[]};Object.assign(window.firebaseCache,{communityPosts:[],users:[],reports:[],festivals:null,meals:null,notices:null,dailySchedules:null,guestAccess:null,reservations:null,reservationSlots:[],timetables:[],classChatMessages:[],schoolMaps:[],suggestions:[],suggestionMessages:[],guestProfiles:[],queueEntries:[],adminReservationStats:null,reservationServerStatus:'checking',notifications:[],personalNotifications:[],notificationStates:{},notificationPreference:{enabled:true},media:{},ready:false})}
...
function listenShared(role){
  ...
  listenerStops.push(onSnapshot(
    ...
    snapshot=>{
      ...
      if(role==='admin'){
        ui().renderAdmin();
        if(!reservationSlotSyncStarted){
          reservationSlotSyncStarted=true;
          syncReservationSlots(window.firebaseCache.festivals,{quiet:true})
            .catch(error=>console.warn('Initial reservation server sync failed:',error));
        }
      }
    },
    error=>ui().toast(displayFirebaseError(error))
...
window.baemoonFirebase={
  ...
  async refreshReservationData(){return refreshReservationServerData(ui().session()?.role)},
  async getSlotStatus(payload){return refreshServerSlotStatus(payload.festivalId,payload.boothId,payload.time||'즉시 예약')},
  async syncReservationSlots(items){return syncReservationSlots(items||window.firebaseCache.festivals||[])},
  async getAdminStats(){requireAdmin();await refreshReservationServerData('admin');return window.firebaseCache.adminReservationStats},
