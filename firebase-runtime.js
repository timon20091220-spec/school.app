...
function displayFirebaseError(error){const code=String(error?.code||"");const message=String(error?.message||"");if(message.includes("DUPLICATE_RESERVATION"))return "이미 같은 부스와 시간으로 예약했습니다.";if(code.includes("network-request-failed")||code.includes("unavailable"))return "인터넷 연결을 확인해주세요.";if(code.includes("too-many-requests"))return "요청이 많습니다. 잠시 후 다시 시도해주세요.";if(code.includes("invalid-credential")||code.includes("wrong-password"))return "로그인 정보가 올바르지 않습니다.";if(code.includes("email-already-in-use"))return "이미 등록된 학생 계정입니다.";if(code.includes("reservation-permission"))return "예약 권한 규칙을 적용하지 못했습니다. v11.3 firestore.rules를 게시해주세요.";if(code.includes("permission-denied"))return "최신 v11.3 Firestore 규칙을 게시해주세요.";if(code.includes("resource-exhausted")||message.includes("용량"))return "사진 또는 데이터 용량이 너무 큽니다.";return message||`Firebase 오류가 발생했습니다. (${code||"unknown"})`}
...
async function finishStudentLogin(user,expected){const ref=doc(db,"users",user.uid);const snap=await getDoc(ref);let profile;if(!snap.exists()){profile={uid:user.uid,role:"student",studentKey:expected.key,grade:expected.grade,classNo:expected.classNo,number:expected.number,name:expected.name,active:true,createdAt:Date.now(),lastLoginAt:Date.now()};await setDoc(ref,profile)}else{profile={uid:user.uid,...snap.data()};const mismatch=profile.studentKey!==expected.key||Number(profile.grade)!==expected.grade||Number(profile.classNo)!==expected.classNo||Number(profile.number)!==expected.number||String(profile.name).trim()!==expected.name;if(mismatch){await signOut(auth);throw new Error("STUDENT_INFO_MISMATCH")}if(profile.active===false){await signOut(auth);throw new Error("STUDENT_SUSPENDED")}await updateDoc(ref,{lastLoginAt:Date.now()});profile.lastLoginAt=Date.now()}ui().setSession(studentSession(profile));const migrated=await migrateStudentLocalData(profile);startDataListeners("student",profile);ui().closeOverlay("studentLoginModal");ui().enterApp("home");ui().toast(migrated?`기기 데이터 ${migrated}개를 Firebase로 이전했습니다.`:`${profile.name}님, Firebase 로그인에 성공했습니다.`)}
async function submitStudentLogin(){const button=$("#studentLoginSubmit"),errorBox=$("#studentLoginError");errorBox.hidden=true;const grade=Number($("#studentGrade").value),classNo=Number($("#studentClass").value),number=Number($("#studentNumber").value),name=$("#studentName").value.trim(),password=$("#studentPassword").value.trim();if(!name||!password){errorBox.textContent="이름과 비밀번호를 입력해주세요.";errorBox.hidden=false;return}if(password!==COMMON_PASSWORD){errorBox.textContent="공통 비밀번호가 올바르지 않습니다.";errorBox.hidden=false;return}const key=studentKey(grade,classNo,number),email=studentEmail(key),expected={key,grade,classNo,number,name};button.disabled=true;button.textContent="Firebase 확인 중…";authFlowInProgress=true;try{let credential;try{credential=await signInWithEmailAndPassword(auth,email,password)}catch(error){const code=String(error.code||"");if(code.includes("invalid-credential")||code.includes("user-not-found"))credential=await createUserWithEmailAndPassword(auth,email,password);else throw error}await finishStudentLogin(credential.user,expected)}catch(error){if(error.message==="STUDENT_INFO_MISMATCH")errorBox.textContent="이미 등록된 학번의 이름과 일치하지 않습니다.";else if(error.message==="STUDENT_SUSPENDED")errorBox.textContent="관리자에 의해 정지된 계정입니다.";else errorBox.textContent=displayFirebaseError(error);errorBox.hidden=false}finally{authFlowInProgress=false;button.disabled=false;button.textContent="학생 로그인";$("#studentPassword").value=""}}
async function submitAdminLogin(){const button=$("#adminLoginSubmit"),errorBox=$("#adminLoginError"),adminId=$("#adminLoginId").value.trim().toLowerCase(),password=$("#adminLoginPassword").value;errorBox.hidden=true;if(adminId!=="admin"){errorBox.textContent="관리자 ID가 올바르지 않습니다.";errorBox.hidden=false;return}button.disabled=true;button.textContent="관리자 확인 중…";authFlowInProgress=true;try{const credential=await signInWithEmailAndPassword(auth,ADMIN_EMAIL,password);if(credential.user.email!==ADMIN_EMAIL)throw new Error("NOT_ADMIN");ui().setSession({role:"admin",uid:credential.user.uid,id:"admin",name:"관리자"});const migrated=await migrateAdminLocalData();startDataListeners("admin");ui().closeOverlay("adminLoginModal");ui().enterApp("admin");ui().toast(migrated?`기존 관리자 데이터 ${migrated}개를 Firebase로 이전했습니다.`:"Firebase 관리자 로그인에 성공했습니다.")}catch(error){errorBox.textContent=error.message==="NOT_ADMIN"?"관리자 계정이 아닙니다.":displayFirebaseError(error);errorBox.hidden=false}finally{authFlowInProgress=false;button.disabled=false;button.textContent="관리자 로그인";$("#adminLoginPassword").value=""}}
async function submitGuestLogin(){authFlowInProgress=true;try{const c=await signInAnonymously(auth);ui().setSession({role:"guest",uid:c.user.uid,name:"게스트"});startDataListeners("guest");ui().closeOverlay("guestConfirmModal");ui().enterApp("home");ui().toast("Firebase 게스트 계정으로 입장했습니다.")}catch(e){ui().toast(displayFirebaseError(e))}finally{authFlowInProgress=false}}
async function logout(){try{await signOut(auth)}finally{stopListeners();ui().clearSession();ui().showAuthGate();ui().toast("로그아웃되었습니다.")}}
function interceptClick(selector,handler){document.addEventListener("click",event=>{const target=event.target.closest(selector);if(!target)return;event.preventDefault();event.stopImmediatePropagation();Promise.resolve(handler(target,event)).catch(e=>ui().toast(displayFirebaseError(e)))},true)}
...
onAuthStateChanged(auth,async user=>{if(authFlowInProgress)return;stopListeners();if(!user){ui().clearSession();ui().showAuthGate();return}try{if(user.isAnonymous){ui().setSession({role:"guest",uid:user.uid,name:"게스트"});startDataListeners("guest");ui().enterApp("home");return}if(user.email===ADMIN_EMAIL){ui().setSession({role:"admin",uid:user.uid,id:"admin",name:"관리자"});const migrated=await migrateAdminLocalData();startDataListeners("admin");ui().enterApp("admin");if(migrated)ui().toast(`기존 관리자 데이터 ${migrated}개를 Firebase로 이전했습니다.`);return}const profile=await getStudentProfile(user);if(!profile||profile.active===false){
  await signOut(auth);
  ui().clearSession();
  ui().showAuthGate();
  ui().toast(profile?.active===false
    ?"정지된 계정입니다."
    :"학생 정보 문서가 삭제되었습니다. 같은 학년·반·번호·이름으로 다시 로그인하면 자동 복구됩니다.");
  return;
}ui().setSession(studentSession(profile));const migrated=await migrateStudentLocalData(profile);startDataListeners("student",profile);ui().enterApp("home");if(migrated)ui().toast(`기기 데이터 ${migrated}개를 Firebase로 이전했습니다.`)}catch(e){ui().clearSession();ui().showAuthGate();ui().toast(displayFirebaseError(e))}});
